 ...
 *
 * Mã OTP biết trước nhờ `OTP_TEST_FIXED_CODE` (playwright.a0.config.ts) — hash là
 * HMAC một chiều nên không đọc ngược từ DB được. Kênh gửi (ZNS) KHÔNG cấu hình ở
 * môi trường test nên delivery thất bại; điều đó CỐ Ý không cản test: `requestOtp`
 * vẫn tạo `OtpRequest` trước khi gửi, và đây là ca kiểm luồng kích hoạt chứ không
 * phải ca kiểm đường dây Zalo (ca đó là smoke thật trên prod).
 *
...
const FIXED_CODE = "424242"; // khớp OTP_TEST_FIXED_CODE trong config
...
test.describe("[P5] Kích hoạt tài khoản bằng SĐT", () => {
  ...
  test("[P5-C1] phụ huynh KHÔNG có email kích hoạt trọn vẹn bằng SĐT rồi đăng nhập được", async ({
    ...
    await expect
      .poll(
        async () =>
          db.otpRequest.count({ where: { target: PHONE, purpose: "ACTIVATION" } }),
        { timeout: 15_000 },
      )
      .toBe(1);
  ...
  test("[P5-C4] hồi quy: tài khoản cũ khoá EMAIL vẫn kích hoạt bằng email như trước", async ({
    ...
    await expect
      .poll(async () => db.otpRequest.count({ where: { target: email } }), { timeout: 15_000 })
      .toBe(1);
