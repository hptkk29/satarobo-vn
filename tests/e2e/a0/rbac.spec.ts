...
test.describe("[A0-02] RBAC động", () => {
  ...
  test("[A0-02-T1-02] seedRoles → đúng 18 role, KHÔNG có HO_MANAGER (AC8)", async () => {
    await seedRoles();
    ...
    //    15 → 18 (25/09/2026): Cổng dữ liệu agent thêm GIAM_DOC (duyệt) + KY_THUAT (tạo)
    //    + AGENT_CHI_DOC (vai của USER DỊCH VỤ, chỉ quyền đọc) — spec CEO §5.4.
    expect(await db.roleDef.count()).toBe(ROLE_SEED.length);
    expect(await db.roleDef.count()).toBe(18);
