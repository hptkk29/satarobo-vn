...
- **Nợ KẾ THỪA ghi nhận (không thuộc US-03, có từ #11):** search oracle — actor không có quyền PII vẫn dò được SĐT qua ô search (`students/page.tsx` baseFilters.OR chứa parentPhone/phone vô điều kiện; cùng lớp ở `enrollments/page.tsx` + `admin/search/page.tsx`). **Nợ MỚI ghi nhận:** actor bị DENY cấp trường thấy form edit prefill giá trị mask — bấm lưu sẽ ghi chuỗi mask vào DB (write path chưa chặn); fieldMask mới nhắm `parentPhone` (parent2Phone/phone HV chưa thuộc phạm vi).
