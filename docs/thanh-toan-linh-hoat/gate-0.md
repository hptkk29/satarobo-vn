...
| **X5** | Công tắc là `SystemSetting billing.flexV1Enabled` **theo `orgUnitId`** (variables.md, US-05/AC3) | `model SystemSetting { key String @id … }` — khoá là **chuỗi phẳng**, không có chiều đơn vị | "Bật cho một cơ sở" không khai được trực tiếp. Hoặc `billing.flexV1Enabled.<orgUnitCode>`, hoặc `valueJson` là danh sách mã cơ sở. **Phải chốt trước US-05** |
...
| # | Việc | Chặn story nào |
|---|---|---|
| 1 | **Công tắc ở đâu** — `SystemSetting` khoá phẳng (kèm cách khai theo cơ sở) hay giữ env `PAYMENT_PER_CHILD_ENABLED` đã có? | US-05 |
