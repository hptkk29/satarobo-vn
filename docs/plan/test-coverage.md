...
| **L-A6** | Cổng **GHI** (điểm danh, chốt buổi, nhận xét, chấm năng lực) chấp nhận cơ sở mà người đó **đang giữ chính vai `CENTER_MANAGER`** — `roleManagesCenter` (`lib/auth/managed-centers.ts`), **KHÔNG** so `session.user.centerId` và **cũng KHÔNG** dùng `visibleCenterIds`. ⚠️ Sửa 26/08/2026: bản đầu dùng `visibleCenterIds AND passesScope` **SAI** — cả hai vế đều nở theo vai kiêm nhiệm nên phép AND không cắt gì (QLCS@CS1 kiêm kế toán@CS2 ghi được lên CS2) | A-01-6 · RT-1 |
...
| **L-A11** | `roles:assign` mở cho `HO_HR` (OQ-7, 24/08/2026): **R1** actor không phải SUPER_ADMIN thì **không** gán được vai chứa `roles:*`/`users:manage` · **R2** không tự gán cho chính mình · **R3** thiếu `reason` ⇒ từ chối. Rào sẵn có SEC-M13 (chỉ SUPER_ADMIN gán được vai `SUPER_ADMIN`) vẫn phải xanh | `A-nen-tang.md` §6.10 |
| **L-A13** | 🔴 **Không nghiệm thu A-01 bằng tài khoản có `SUPER_ADMIN`.** Fixture e2e phải là QLCS **thuần** giữ 2 cơ sở khác vùng; thêm một ca đối chứng: cùng kịch bản với tài khoản `SUPER_ADMIN` thì **luôn thấy hết** — để bộ test tự chứng minh vì sao fixture kia là bắt buộc | `A-nen-tang.md` §6.9.1 |
| **L-A14** | Gỡ/hết hạn dòng `UserOrgRole` `CENTER_MANAGER` của một người **cũng là `SUPER_ADMIN`** ⇒ người đó **rời nhóm chat lớp** của cơ sở đó (dashboard vẫn đủ). Test phải khẳng định **cả hai vế**, vì đây là dạng hỏng âm thầm mà SL-01 sinh ra | `lib/chat/sync-membership.ts:155-158` · §6.9.1 |
| **L-A12** | Bộ lọc A-02 (OQ-4, 24/08/2026): `groupByCenter=false` (mặc định) trả **một tập số**; `=true` trả **mỗi cơ sở một dòng + dòng Tổng**, và **tổng các dòng = số gộp**. Công tắc **không hiện** khi chỉ chọn 1 cơ sở | `A-nen-tang.md` §6.2 |
...
| **L-F9** | Retention: điều kiện là **`ReportCard.sentToParentAt IS NOT NULL`** ("đã gửi đến PH" — quyết định B6 ngày 24/08/2026), **không** phải `status = PUBLISHED`. Học bạ `PUBLISHED` mà chưa gửi ⇒ **không xoá**, ghi log kèm `reportCardId`; `RECALLED` ⇒ không xoá; không xác định được ⇒ **GIỮ** | F-05-2 · §6.1.5(3) · Story 18 |
...
| **L-G12** | Đánh dấu rớt: `LeadChild.status = LOST` (theo con) + **bắt buộc `Lead.lostNote`** (ô ghi chú tự do ở **cấp phụ huynh** — 12(b) + B5 ngày 24/08/2026); **không** còn danh mục `LeadLostReason` | C-06-1 · `CDB-dashboard.md` §C.6.8 |
...
| A-7 | QLCS 2 cơ sở **điểm danh + chốt buổi + nhận xét + chấm năng lực** một lớp ở cơ sở thứ hai | L-A6 | ✅ thành công ở cả hai cơ sở. ❌ **Từ chối:** lớp ở cơ sở thứ ba. ❌ **Từ chối:** người kiêm vai KHÁC ở cơ sở thứ hai (kế toán/marketing) | `app/(admin)/admin/sessions/[id]/_actions.test.ts` (17 ca) · `app/(admin)/admin/students/[id]/_actions.test.ts` (11 ca) · `lib/lms/skill-access.test.ts` (11 ca) | unit + integration | **existing** (26/08/2026) · ⏳ còn e2e trình duyệt |
...
| F-30 | Media > 12 tháng, học bạ liên kết `PENDING_REVIEW` **hoặc** `PUBLISHED` mà `sentToParentAt IS NULL` | **L-F9** | ❌ **Từ chối xoá:** `decision = KEPT_REPORT_CARD_NOT_SENT` + `reportCardId` + `reportCardStatus` + `sentToParentAt` đọc được | Doc F-05-2 §6.1.5(3); Story 18; quyết định B6 | unit | **proposed** |
...
| G-24 | Đánh dấu rớt **thiếu `lostNote`** (rỗng hoặc chỉ khoảng trắng) | **L-G12** | ❌ **Từ chối:** Server Action trả lỗi *"Bắt buộc nhập lý do rớt"*; **không** ghi `LeadChild.status` | `CDB-dashboard.md` §C.6.8 (`markChildLostSchema`) | integration | **proposed** |
| G-25 | Đánh dấu rớt với `lostNote` dài **> 2000 ký tự** | L-G12 | ❌ **Từ chối** ở tầng Zod, không cắt ngầm | §C.6.8 (`max(2000)`) | unit | **proposed** |
| G-26 | PH **hai con đều `LOST`**, gỡ **một** con khỏi `LOST` | L-G12 (bẫy B5) | ✅ `Lead.lostNote`/`lostAt` **GIỮ NGUYÊN** (đứa còn lại vẫn rớt). Chỉ khi gỡ **con cuối cùng** mới xoá, trong cùng transaction | §C.6.8 bẫy B4 (bản B5) | integration | **proposed** |
...
| `[F-05] decideMediaRetention` | 5 ca: chưa tới hạn → `KEPT_NOT_DUE`; học bạ `PENDING_REVIEW` → `KEPT_REPORT_CARD_NOT_SENT`; học bạ `PUBLISHED` **nhưng `sentToParentAt IS NULL`** → `KEPT_REPORT_CARD_NOT_SENT` (ca **mới theo B6** — trước đây ca này bị xoá nhầm); `RECALLED` → **giữ**; mọi học bạ có `sentToParentAt` → `DELETED` | Ca `linkedReportCards = []` — 🔴 **chặn bởi OQ-F4**, không viết cho tới khi chốt (xem F-32 ở §3.F) | unit |
...
| `[C-06] rớt bắt buộc ghi chú lý do` | `lostNote` rỗng / chỉ khoảng trắng / > 2000 ký tự; và ca "hai con cùng rớt, gỡ một con" | Ba ca đầu **từ chối**; ca cuối **không xoá** `Lead.lostNote` | integration |
...
| **3** | **L-F9** — retention xoá ảnh trẻ em trên điều kiện "học bạ đã xuất" | Xoá hàng loạt, **không hoàn tác**; hoặc không xoá gì mà tưởng đã tuân thủ | `F-media.md` §6.1.5(1); T4; Story 8 trước Story 18 | ✅ **Điều kiện đã chốt 24/08/2026 (B6): `sentToParentAt IS NOT NULL`** — chặt hơn `PUBLISHED`, và học bạ cũ (`sentToParentAt = null`) **không bị xoá media**. ⚠️ Vẫn còn **OQ-F4** mâu thuẫn giữa 2 PRD |
...
| ~~**14**~~ | ~~L-A1/L-A6 — QLCS đa cơ sở đọc được nhưng không ghi được~~ | ✅ **ĐÃ ĐÓNG 26/08/2026** cho 5 cổng trên đường điểm danh/chốt buổi/nhận xét/chấm năng lực (+39 test). 🔴 **CHƯA đóng** cho ~13 cổng GHI còn lại cùng khuôn — xem `docs/plan/ket-va-cach-go.md` K-18 | `A-nen-tang.md` RT-1 | Fixture QLCS thuần 2 cơ sở khác vùng đã có (`tests/e2e/_helpers/seed-multi-region.ts`) |
