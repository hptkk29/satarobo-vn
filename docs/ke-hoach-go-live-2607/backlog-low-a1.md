...
## 1. ✅ ĐÃ XỬ 10/07 (Kiệt chọn phương án b) — hội thoại đọc THEO HỌC VIÊN
...
(c) Chấp nhận + quy trình tay. **→ Kiệt chọn (b) 10/07, đã code:** `getThreadForStudent`/`markStudentThreadRead`/
`listStudentThreadSummaries` (lib/conversation/service.ts); admin inbox theo HV (entitlement = sdb.student,
tiền lệ câu 20; GV giới hạn classIds — câu 19; trả lời vào enrollment hiện hành); portal giữ URL ?e= nhưng
luồng + badge gộp theo con. e2e r7 conversation-by-student 5/5.
