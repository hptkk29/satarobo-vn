...
> **Ngày lập:** 05/09/2026 (bản 2, sau khi chủ dự án chốt 13 câu chiều 05/09) · **Nhánh:** `feat/zalocrm` (cắt từ `origin/test`).
> **Trạng thái 06/09/2026:** 🟢 **PHẦN SATA (S1–S12) ĐÃ CODE XONG** — 4 commit trên `feat/zalocrm`, mọi thứ nằm sau cờ
> `ZALOCRM_ENABLED` mặc định TẮT. Chi tiết việc đã làm + việc còn lại: [`03-da-lam-va-con-lai.md`](03-da-lam-va-con-lai.md).
> Bản đặc tả thi công dùng để xây: [`02-dac-ta-thi-cong-phan-sata.md`](02-dac-ta-thi-cong-phan-sata.md).
> **Phần fork ZaloCRM (F1–F7) CHƯA làm** — đó là repo khác, và chặn ở ba việc ngoài code của chủ dự án (9.16 SIM thử,
> 9.17 máy chủ, 9.18 tên sản phẩm mới).
...
| **S6** | **Quyền:** action mới `zalocrm:use` khai ở 3 nơi (`lib/auth/permissions.ts` v1, `prisma/seed-roles.ts` v2 ~~scope CENTER~~ **[ĐẢO 06/09 khi thi công — scope GLOBAL]**, `lib/permissions/registry/`)<br>⚠️ **Vì sao đảo:** `lib/auth/can.ts` nhánh `case "CENTER"` mở đầu bằng `if (!target?.centerId) return false`. Cổng trang gọi `checkPermission("zalocrm:use")` **không truyền target** ⇒ CENTER luôn trả false ⇒ trên PROD (RBAC v2 đang bật) mọi vai trừ SUPER_ADMIN bị đá khỏi `/zalo-crm`, mà LOCAL chạy v1 tĩnh nên vẫn xanh — hỏng câm và không tái hiện được ở máy dev. Cách ly cơ sở **không** đến từ `scopeType` mà từ `scopedDb`/`where`, cấp cho SUPER_ADMIN, CENTER_MANAGER, CENTER_CLASS_MANAGER, SALES_CSM; test parity. Vai → vai ZaloCRM: SUPER_ADMIN/CENTER_MANAGER/CENTER_CLASS_MANAGER ⇒ `admin`, SALES_CSM ⇒ `member`. Sau merge main: chạy `seed-prod-roles.yml`. | 3 file quyền + `lib/integrations/zalocrm/sso.ts` | Không dùng grant DENY. |
