...
## 1.5 "Của tôi" — `me-nav.tsx` (RSC, admin-only): tab Lịch ca `"/cham-cong/lich-ca"` · Đơn của tôi `"/don-tu/cua-toi"` + nút BTN_OUTLINE "Chấm công" `"/cham-cong/checkin"`. Không ScopeBar. **`?month=` đi theo CẢ HAI tab** — bấm sang "Đơn của tôi" rồi quay lại phải còn đúng tháng đang xem. **Kiosk** `/cham-cong/man-hinh?centerId=`: hai chế độ một URL — §4.
...
ModuleNav / ScopeBar · KpiStrip Công chuẩn 22 (hint "30 − 8 nghỉ tuần − 0 lễ") · Tổng công 402 · Ngày có cờ 7 (W) · Buổi dạy ở cơ sở này 118 · Người 19
...
SectionCard "Công chuẩn & trạng thái" (period-panel.tsx) · Bảng 2 hàng: NHÂN SỰ | CÔNG (Công · KH · Nghỉ CL · Lễ) | GIỜ (HC · Làm/KH) | HẬU KIỂM (Muộn · Sớm · Không lượt · Ghi đè · Cờ) | BUỔI Ở CƠ SỞ + tfoot
Chân "Số đã chốt lúc 01/10 09:12 — lý do — đã xuất 2 lần" / "Bản tạm dựng 10:31 · cập nhật vài phút sau mỗi lượt quét"
```
- `period-table.tsx`: `<colgroup>` + `<thead>` 2 `<tr>` (nhóm `colSpan scope="colgroup"` `text-[11px] uppercase`); Nhân sự sticky 1 dòng (tên truncate + mã NV mono nhỏ); Công & Buổi ở cơ sở `font-semibold`; ô Cờ/Ghi đè = `<Link>` `/cham-cong?date=<ngày đầu có cờ>&coSo&q=<tên>`; `expectedUnits === 0` ⇒ pill M "Chưa có ca". `PhanTrangBang khoaGhiNho="ky-cong" soDongMacDinh={50}`.
- ⚠️ **HAI con số buổi dạy, cố ý khác nhau — đừng đặt trùng tên, đừng sửa cho khớp.** Kỳ công đếm buổi của lớp **thuộc cơ sở đang xem** (`buildPeriodSummary` lọc `class.centerId`) ⇒ nhãn **"Buổi ở cơ sở"**; màn Công dạy đếm theo **người** (`loadBuoiDay` hỏi theo `userId`) ⇒ nhãn **"Tổng buổi của người"**. Giáo viên dạy chéo cơ sở là chỗ hai số tách nhau, và lệch ở đó là ĐÚNG. Cả hai màn có một đoạn `PageHelp` nói điều này — sửa nhãn thì sửa cả đoạn đó, và cả tiêu đề cột Excel (`export-xlsx.ts`). Tên TRƯỜNG `teachingSessions` thì KHÔNG đổi: nó đã đóng băng trong `AttendancePeriod.summaryJson` của các kỳ đã chốt.
...
- **Điều khiển** (`max-w-3xl`): PageHeader "Màn hình QR" subtitle "Mở trên TV tại quầy — cùng mã với tờ dán ở quầy" [Trình chiếu ● `Monitor`] · ScopeBar `paramName="centerId"` chip = cơ sở `code ∈ byCode` (loại HO) có `view`; `centerId=hoi-so` ⇒ EmptyState "Hội sở không có điểm chấm công (Q-04)" · `lg:grid-cols-2 gap-5`: trái `QrScreen` 240px; phải SectionCard "Điểm chấm công" `<dl>` (tên điểm · Định vị · đời khoá mã · link "Sửa điểm chấm công" khi config) · dưới SectionCard "Lượt chấm hôm nay" (`today-taps.tsx`: gate view, `staffTimeLog` ACCEPTED hôm nay qua sdb, NHÂN SỰ | GIỜ | VÀO/RA | CỜ, PhanTrangBang) — **chỉ chế độ này**.
...
- **Chống chết lặng** [ĐỔI 07/09 — mã TĨNH]: mã không hết hạn nên KHÔNG còn `validUntil`/`windowSeconds`/đếm ngược. Có ảnh là còn quét được, bất kể poll hỏng bao lâu; poll 30s `cache:"no-store"` chỉ để tự hồi phục sau rớt mạng và để đổi ảnh sau khi admin thu hồi mã. Poll lỗi ⇒ giữ QR + dải W `text-2xl` "Mất kết nối 14:03 — mã vẫn quét được bình thường"; CHƯA BAO GIỜ lấy được ảnh nào mới ⇒ `ErrorState` cỡ TV + Thử lại h-11 + tự thử 30s. **401** ⇒ "Phiên đăng nhập trên TV đã hết" + `/login?callbackUrl=…`; **403** ⇒ `NP("view","màn hình QR")`; **404** ⇒ EmptyState "CS1 chưa có điểm chấm công" + "Tạo điểm chấm công" (`/cham-cong/diem-cham` khi config, không thì "Báo Quản lý cơ sở"); **500** ⇒ "Máy chủ chưa cấu hình khoá ký mã". Tải đầu `Skeleton` vuông.
