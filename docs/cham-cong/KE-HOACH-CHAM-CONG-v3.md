# KẾ HOẠCH MODULE CHẤM CÔNG — bản v3.4 (37 câu chốt · lịch phân ca thật · khảo sát MISA · nguyên tắc tự vận hành · **L0–L6 code xong 06/09**)

> **Ngày lập:** 03/09/2026 (v3) → v3.1 cùng ngày (16 câu + Sheet) → v3.2 (khảo sát MISA 04/09 + 5 file xuất) → v3.3 05/09/2026 (chủ dự án chốt U-01…U-03 + K-01…K-09 + PHẦN 6b) → **v3.4 06/09/2026: L0–L6 đã viết xong code + test (xem §7.2 từng lô), còn việc bấm tay + nghiệm thu + kỳ chạy song song**.
...
**Hệ quả M5 (S-37):** ghép Sheet ↔ tài khoản làm ở **màn import** (L1): hệ thống gợi ý theo `Employee.phone`/`Employee.fullName` (Employee có cột `phone`, User không), người vận hành **xác nhận từng dòng một lần**, lưu ánh xạ vào `ShiftAssignment.userId`/`employeeId` để lần sau tự khớp. Không cần U-03 dưới dạng "danh sách SĐT để dev ghép tay" — đúng PHẦN 6b. **Đo prod 05/09 (lần 2):** `Employee` ACTIVE **20**, **20/20 đã liên kết User**, chỉ **5/20 có SĐT** hợp lệ ⇒ gợi ý ghép ở màn import đi theo **tên** (`Employee.fullName` so khớp tên trong Sheet, bỏ dấu + hoa/thường) là chính, SĐT chỉ là gợi ý phụ; và vì mọi Employee đều có User nên ánh xạ Sheet → Employee là đủ (không cần bước thứ hai sang User). Sheet 19 người ↔ 20 Employee: chênh 1 (Mr Phúc `timesheetExempt` hoặc người mới) — màn import phải hiện "chưa khớp" cả hai chiều. Còn phải đo: **toạ độ thực địa CS1/CS2**.
...
| **L1** (2,5 tuần) → **✅ XONG 06/09/2026** (code) | 10 bảng mới (`LeaveType` thêm) + `WorkRequest`/`Holiday`/`Employee` mở rộng (migration `20260906090000_cham_cong_v3_l1`, thuần additive, đã áp Postgres local) · 5 key mới × 4 nơi khai (+`CENTER_HR` assign/config vì parity v1 HR) · **10 setting `shift.*`** · seed nền `pnpm db:seed:cham-cong` (21 mã + 8 loại nghỉ + WorkLocation CS1/CS2) · **màn `/cham-cong/phan-ca/import`** (ánh xạ tên gợi ý theo tên/SĐT, nhớ qua `sheetName`; khung ca + lưới tháng; giữ ca SWAP/LEAVE/MANUAL; hàng ngoài quyền đếm riêng; đối chiếu 15 con số) · `resolveHomeCenter()` · cờ Miễn tính công trên form nhân sự. **Dời sang L4:** đóng băng 5 bảng cũ + sửa 7 vitest cũ (check-in L0 còn ghi `EmployeeCheckin` tới khi `StaffTimeLog` thay). **`adjust` giữ GLOBAL tới L5** (R1). | `migrate deploy` xanh ✅ · `[US-07-IT-08b]` xanh ✅ · test tích hợp `tests/cham-cong/import.spec.ts`: 15 con số DB = Sheet ✅ · **còn chờ:** seed vai + seed nền bấm tay trên test/prod, QLCS import thật trên test |
| **L2** (1,5 tuần) → **✅ XONG 06/09/2026** | `lib/cham-cong/engine.ts` (thuần, 32 test viết trước: 7 ví dụ §6.3-bis theo giờ Sheet + 21 mã + muộn/sớm/thiếu buổi + lễ + miễn công) · `recompute.ts` (đường ghi duy nhất, LOCKED bỏ qua, giữ `overrideUnits`, `acceptedLogsOfDay`) · DomainEvent `hr.attendance_day_dirty` (dedupeKey theo phút — unique vĩnh viễn) + handler · import/sinh lưới/sửa ô đều xếp hàng tính lại · `tests/cham-cong/recompute.spec.ts` (5 ca DB thật). **Chốt thêm:** `DEN_SAT_GIO` = vào SAU giờ bắt đầu nhưng trong dung sai (đến sớm không cờ); `THIEU_GIO` = hụt > 60′ | ✅ xanh trong job Unit tests (lib/cham-cong nằm trong `include`) |
| **L3** (2,5 tuần) → **✅ XONG 06/09/2026** (code) | `/cham-cong/danh-muc-ca` (CRUD mã ca, gate `config`) · `/cham-cong/khung-ca` (khung ca tuần theo khối + **Sinh lưới**, không đè ô sửa tay/đơn/file, cảnh báo Điều 111) · `/cham-cong/phan-ca?ky&coSo` (lưới tháng ô `<select>` → MANUAL + dirty + **thông báo người đó — T-07**) · `/cham-cong/ghi-chu` (việc cố định/ghi đè) · `Holiday` 4 cột + Kế toán sửa được (gate `holidays:edit` ∨ `hr_attendance:config`, gỡ khỏi inline-authz-allowlist) · cron `/api/cron/shift-brief` **mỗi giờ, tự so giờ VN với setting `shift.briefNoteHourVN`** (đổi giờ gửi không cần dev) + `cron-pump-test.yml` · 5 mục sidebar. **Dời:** nơi làm GV theo `ClassSession` (§4.1) → L5 cùng đơn dạy thay; ZNS thật → sau (chỉ prod). | **còn chờ nghiệm thu:** 2 tuần lịch song song Sheet ↔ hệ thống, tin in-app 19:00 khớp tin Zalo |
...
| **L4** (2 tuần) → **✅ XONG 06/09/2026** (code) — QR quầy XOAY 60s (`kiosk-token.ts`, nhận 2 cửa sổ trước) · vé 120s tiêu nguyên tử (`timelog.ts`) · `recordTimeLog` GHI LUÔN + cờ (Q-07), `SAI_NOI_LAM` chỉ AT_UNITS (§4.10), gác chéo cơ sở L0 gỡ · check-in admin + site GV cùng cổng `checkin-gate.ts` · `EmployeeCheckin` không còn được ghi · `/cham-cong` = bảng công ngày trên bảng mới · `/cham-cong/diem-cham` · test kiosk (4) + `timelog.spec.ts` (3 ca DB). **Còn chờ nghiệm thu thật:** 7 ví dụ §6.3-bis trên Android + iPhone; đo toạ độ CS1/CS2 rồi bật geofence từng cơ sở. **Kế hoạch gốc:** | `WorkLocation` + vé 120s + `recordTimeLog` + **màn chấm công trên site GV** · đóng `/cham-cong/checkin`, `/man-hinh`, `api/qr-token`, `lib/attendance/qr.ts` **cùng lúc** · bật `geofenceEnabled` **từng cơ sở, mỗi tuần một** | 7 ví dụ §6.3-bis trên Android + iPhone · người làm S+C/C+T chấm đủ |
| **L5** (2 tuần) → **✅ XONG 06/09/2026** (code) | **A. Đơn từ dùng chung** `lib/cham-cong/requests.ts` + `request-actions.ts` ("use server" dùng chung admin+GV, tiền lệ `lead-intake`) + `components/cham-cong/request-form.tsx`: mọi nhân sự nộp được (`/don-tu/cua-toi` admin, `/teacher/don-tu` GV mount cùng form) · luật **cơ sở nhận đơn** = ca ngày áp dụng → cơ sở nhà → HO phải chọn · nộp muộn theo `shift.requestNoticeDays` (cờ, không chặn) · chặn đơn trùng + kỳ đã khoá · **duyệt = áp trong `db.$transaction`** (SHIFT_SWAP/LEAVE qua `setAssignmentCell` nguồn SWAP/LEAVE, TIMESHEET_FIX ghi `StaffTimeLog MANUAL_ADJUST` + dirty; khoá lạc quan `updateMany(status=PENDING)`; `applyError` ghi SAU rollback — T-05) · đơn lớp (CLASS_OFF/SUB_TEACH) **bù trừ về PENDING** khi `applyApprovedWorkRequest` thất bại (không luồn `tx` vào `lib/classes/adjust.ts` — hàm đó tự mở tx, sửa là chạm luồng lớp) · gate `hr_attendance:approve` **theo cơ sở nhận đơn** (bỏ `hasRole` + so `centerId`, gỡ allowlist inline-authz) · thông báo `request.submitted:` (người có approve tại cơ sở, đọc `UserOrgRole`×`RolePermission`) + `request.decided:` · **B. Kỳ công** `period.ts`: công chuẩn tự sinh (ngày − nghỉ tuần − lễ, K-04, Kế toán sửa trước khoá), `buildPeriodSummary` (Σ công đã tính ghi đè, cờ, **buổi dạy = ClassSession COMPLETED thực dạy** — K-05), `lockPeriod` (recompute → LOCKED + `summaryJson` + đóng băng ngày; kỳ tương lai không chốt), `reopenPeriod` (cấp Hội sở: `close-period` @ hoi-so), `setDayOverride` (hộp cờ QLCS, lý do bắt buộc, ngày LOCKED chặn) · `/cham-cong/ky-cong` · **xuất Excel** `GET /api/admin/cham-cong/export` (`hr_attendance:export`, kỳ chốt đọc `summaryJson`, audit + `exportCount`, mã NV ép chuỗi, 3 sheet) · `/cham-cong/loai-nghi` (LeaveType CRUD, K-06) · **C. Đóng băng**: gỡ `/cham-cong/{duyet-ca,chinh-cong,yeu-cau-cong,lich-ca-nhan-vien}`, API `shift-export`, `lib/attendance/{adjust,own-adjustments,shift-config,shift-excel}`; `/cham-cong/lich-ca` viết lại đọc lưới mới (giữ href cho thông báo); `/teacher/bang-cong` + `/teacher/lich` đọc `ShiftAssignment`/`StaffAttendanceDay`/`WorkRequest`; pending-tasks nhóm "Đơn từ chờ duyệt" (gác `approve` có target); **`adjust` hạ GLOBAL → CENTER** (call-site trần đã gỡ, rbac-scope R1 xanh) · **Phát hiện:** `publishEvent` bắt P2002 rồi `findUnique` nhưng TRONG tx Postgres đã hỏng (25P02) ⇒ `markAttendanceDayDirty` chuyển sang `createMany skipDuplicates` | `tests/cham-cong/requests.spec.ts` (10) + `period.spec.ts` (6) xanh trên Postgres local ✅ · rbac-scope/parity/page-gates/nav/bang xanh ✅ · **còn chờ:** seed vai bấm tay (adjust CENTER + approve/close-period/export mới), nghiệm thu UAT trên test |
| **L6** (1 kỳ) → **✅ CÔNG CỤ XONG 06/09/2026** — kỳ chạy song song **chưa bắt đầu** (lịch: cutover 01/12) | Lõi thuần `lib/cham-cong/reconcile.ts` (so từng người × ngày: ô lịch Sheet đã gộp D1/D2 vs `ShiftAssignment`, công Sheet 1 ô = 1 vs `StaffAttendanceDay`, tổng tháng; `P`/`X` hệ thống mà Sheet trống KHÔNG tính lệch; miễn công loại tường minh; chưa ánh xạ báo riêng; **chuỗi ngày sạch** đếm từ cuối) · hai vỏ: màn `/cham-cong/doi-soat` (upload Sheet, chỉ đọc, gate `view`) + `pnpm tsx scripts/cham-cong-doi-soat.ts <file.xlsx> [--ky]` (exit 1 khi lệch — cắm workflow đêm được) · không import lịch sử (K-07) | `tests/cham-cong/reconcile.spec.ts` (5) ✅ · **cổng ra thật** (0 lệch 10 ngày làm việc liên tiếp) chỉ đo được khi chạy song song kỳ 12/2026 |
| **L7 — Thiết kế lại giao diện** (06/09/2026, ngoài lộ trình gốc) → **✅ CODE XONG** | Chủ dự án yêu cầu thiết kế lại TOÀN BỘ màn admin của module (site GV giữ nguyên, chỉ đổi theo component dùng chung). Quy trình: phê bình 8 cụm màn (Nielsen + 4 trạng thái) → đo ràng buộc (test/lint/RSC/behavior contract) → hội đồng 3 đặc tả × 2 giám khảo → **đặc tả chốt `docs/cham-cong/DESIGN-CHAM-CONG-ADMIN.md`** (luận đề "Sổ kỳ công": mọi màn chia sẻ khung KỲ = tháng × khối). Kết quả: **sidebar 15 → 5 mục** + `ModuleNav`/`ConfigTabs`/`MeNav` (route KHÔNG đổi) · `ScopeBar` giữ `ky`/`coSo`/`date` khi đổi tab-khối-tháng · `DayStrip` 31 ô đếm cờ · 13 màn có đủ 4 trạng thái (`loading.tsx`+`error.tsx` cho 15 thư mục, `NoPermission` có key + hỏi ai thay redirect câm) · bảng về 44px `adminTh/adminTd` + `whitespace-nowrap` · màu chỉ qua token (hết `amber/violet/sky/rose`) · sửa 6 bug P0 thật: rơi `coSo` khi đổi ngày · ô `→CS2` đổ vào `<select>` thành ô trống giả · `applyImportAction` ném lỗi không ai bắt · nghỉ tuần T2 cứng trong UI · kiosk render trong khung admin (lộ menu quản trị ra phòng chờ) · xoá ghi chú lịch không xác nhận. **Nợ mở:** `Holiday` chưa vào `NULL_IS_GLOBAL_MODELS` nên lễ toàn hệ thống tàng hình với người cấp cơ sở (DayStrip tạm bù bằng `dayType`); `StaffTimeLog` không đọc được bằng vai `checkin` nên màn "Của tôi" hiện công ngày thay vì mốc quét. | typecheck + lint 0 lỗi · 906 test xanh (gồm 33 test DB `tests/cham-cong`) · nav-coverage/bang-coverage/page-gates/menu-permissions/rbac-scope xanh · detector impeccable 0 lỗi thật |
