...
> # 🏷️ C ĐANG THỬ — ĐỪNG DÙNG CHO PHỤ HUYNH THẬT
>
> Đường này **chưa từng chạy với tiền thật**. Phải làm xong **mục C** (thử tiền thật số nhỏ)
> trước khi sale CS2 bấm "In QR" cho một gia đình có thật.
>
> Và tính tới 21/09 nó **chưa lên prod** — PR #317 nằm ở nhánh `test`. Xem đầu mục C.
...
## C · THỬ TIỀN THẬT SỐ NHỎ — làm TRƯỚC khi cho sale dùng  🏷️ **C ĐANG THỬ**

> # ⛔ CHƯA LÊN PROD — ĐỌC TRƯỚC KHI LÀM BẤT CỨ BƯỚC NÀO Ở ĐÂY
>
> PR #317 đã merge vào nhánh **`test`**, **KHÔNG phải `main`**. Cổng mới `Nguồn phải là nhánh
> test` (PR #316, thêm 20/09) chặn mọi PR đi thẳng `main`.
>
> **Hệ quả tốt:** nỗi lo *"merge xong nút In QR hiện ngay với sale CS2"* **KHÔNG xảy ra**.
> Trên `admin.satarobo.vn` hiện **chưa có** nút "In QR" phiếu gộp, và webhook prod **chưa** có
> đường tự chia theo phiếu. Prod hôm nay hành xử y như trước.
>
> **Hệ quả xấu:** mục C này **chưa chạy được**. Thử tiền thật cần SePay bắn vào **prod** —
> `test.satarobo.vn` không nhận được giao dịch ngân hàng thật.
>
> Muốn đưa lên prod thì **chủ dự án chọn một trong hai**, đây là quyết định nghiệp vụ:
>
> | Cách | Kéo theo gì |
> |---|---|
> | PR `test` → `main` (đúng luồng) | mang theo **225 commit** của mọi việc khác đang nằm trên `test` |
> | Nhãn `hotfix-thang-main` trên một PR riêng | chỉ PHIÊN C, nhưng **đây không phải vá gấp** — prod không hỏng. Cửa thoát ấy sinh ra vì #312/#313 của chính tôi, dùng nó cho một tính năng mới là đúng thứ nó được dựng để chặn |
>
> **Chưa chọn thì dừng ở đây.** Mọi bước dưới chỉ chạy được khi đường này đã ở trên prod.

---

### 🏷️ Nhãn C ĐANG THỬ

**Cho tới khi làm xong toàn bộ mục C, sale CS2 KHÔNG dùng nút "In QR" phiếu gộp cho phụ
huynh thật.** Đường này **chưa từng chạy với tiền thật** — không có lượt nào trên prod, không
có giao dịch ngân hàng nào đã đi qua nó.

Đường **gắn tay** và **tách khoản** (phiên trước) vẫn dùng bình thường: chúng đã nghiệm thu
20/09.

---

### C.1 · Chuẩn bị — một đơn thử, hai con, số NHỎ

Chủ dự án tạo (hoặc chỉ định) **một đơn ở CS2 có hai dòng hàng**, rồi tạo cho mỗi bé **một
đợt nhỏ**:
...
> ⚠️ **Dùng đơn THỬ, đừng mượn đơn của phụ huynh thật.** Lượt thử này sẽ để lại trong sổ: 1
> phiếu gộp, 2 phiếu thu PAID, 2 dòng `Payment`, 1–2 `BankTransaction`. Trên đơn thật thì đó
> là rác lẫn vào sổ tiền của một gia đình có thật.
>
> ⚠️ Số nhỏ là **có chủ đích**: nếu đường này sai, thứ sai được là 5.000đ chứ không phải
> 5.000.000đ.

### C.2 · Phát phiếu và KIỂM TRƯỚC KHI chuyển tiền

1. Mở đơn thử → khối **"Công nợ theo con"** → **tick cả hai đợt** → **"In QR 5.000đ"**.
2. Ghi lại **mã 5 ký tự** hiện trên màn.
3. Chạy SQL dưới. Phải ra **đúng một dòng**, `so_dong = 2`, `tong_dong = 5000`:

```sql
SELECT pb."matchKey"        AS ma,
       pb.status            AS trang_thai,
       pb."amountDue"       AS tong_phieu,
       count(pbl.id)        AS so_dong,
       sum(pbl.amount)      AS tong_dong
FROM "PaymentBill" pb
JOIN "PaymentBillLine" pbl ON pbl."billId" = pb.id
JOIN "Order" o             ON o.id = pb."orderId"
WHERE o.code = '<MÃ ĐƠN THỬ>'
  AND pb.status = 'OPEN'
GROUP BY pb.id, pb."matchKey", pb.status, pb."amountDue";
```

> ⛔ **DỪNG nếu** `so_dong ≠ 2` · `tong_dong ≠ 5000` · ra nhiều hơn một dòng · mã có chứa bất
> kỳ ký tự nào trong `O 0 I 1 L B 8 S 5` (bảng chữ đã loại chúng — thấy là có gì đó sai).

### C.3 · CA LỆCH SỐ TRƯỚC — chuyển **4.000đ**

> # ⚠️ THỨ TỰ HAI CA KHÔNG ĐƯỢC ĐỔI, VÀ ĐÂY LÀ LÝ DO
>
> Làm ca đúng số trước thì phiếu thành **PAID**, và lượt 4.000đ sau đó sẽ bị chặn vì
> *"phiếu đã thu đủ"* (`PHIEU_KHONG_MO`) — **không phải** vì lệch số. Cả hai đều về UNMATCHED
> nên nhìn qua tưởng đã kiểm xong, trong khi **nhánh lệch số chưa ai đi vào**.
>
> Ca test chạy đúng thứ tự thì đo được hai nhánh khác nhau. Ca test sai thứ tự đo một nhánh
> hai lần.

1. Chuyển khoản **4.000đ** theo đúng QR / đúng nội dung chuyển khoản (chỉ đổi **số tiền**).
2. Đợi 1–2 phút cho SePay bắn webhook.
3. Kiểm:

```sql
SELECT "providerTxnId", amount, status, left("unmatchedNote", 200) AS ly_do
FROM "BankTransaction"
WHERE amount = 4000
  AND "transferredAt" > now() - interval '2 hours'
ORDER BY "createdAt" DESC
LIMIT 3;
```

**Phải ra:** `status = UNMATCHED`, `ly_do` bắt đầu bằng **`[LECH_SO]`** và nói rõ *thiếu
1.000đ*.

Và **không đồng nào được phân bổ**:

```sql
SELECT count(*) AS so_phan_bo
...
> ⛔ **DỪNG PILOT nếu** `so_phan_bo > 0`, hoặc phiếu đã đổi khỏi `OPEN`, hoặc có dòng
> `Payment` nào sinh ra. Đó là luật *"ăn cả hoặc không ăn gì"* bị thủng — báo ngay.

### C.4 · CA ĐÚNG SỐ — chuyển **5.000đ**

1. Chuyển khoản **5.000đ** theo đúng QR.
2. Đợi 1–2 phút.
3. Một câu gộp trả lời cả cụm, `ket_qua` phải là **`ĐẠT`**:

```sql
SELECT count(*)                                              AS so_khoan,
       count(*) FILTER (WHERE p."orderItemId" IS NOT NULL)   AS so_khoan_co_be,
       count(DISTINCT p."orderItemId")                       AS so_be_nhan,
       sum(p.amount)                                         AS tong_tien,
       (SELECT pb.status FROM "PaymentBill" pb
         WHERE pb."orderId" = o.id ORDER BY pb."createdAt" DESC LIMIT 1) AS trang_thai_phieu,
       CASE WHEN count(*) = 2
             AND count(*) FILTER (WHERE p."orderItemId" IS NOT NULL) = 2
             AND count(DISTINCT p."orderItemId") = 2
             AND sum(p.amount) = 5000
             AND (SELECT pb.status FROM "PaymentBill" pb
                   WHERE pb."orderId" = o.id ORDER BY pb."createdAt" DESC LIMIT 1) = 'PAID'
            THEN 'ĐẠT' ELSE 'KHÔNG ĐẠT — soi từng dòng bằng câu dưới' END AS ket_qua
FROM "Payment" p
JOIN "Order" o ON o.id = p."orderId"
WHERE o.code = '<MÃ ĐƠN THỬ>' AND p."deletedAt" IS NULL;
```

Soi từng dòng khi cần:

```sql
SELECT p.amount, oi."itemName" AS con, p."accountantStatus", p."enrollmentId",
       left(p.note, 120) AS ghi_chu
FROM "Payment" p
JOIN "Order" o             ON o.id = p."orderId"
LEFT JOIN "OrderItem" oi   ON oi.id = p."orderItemId"
WHERE o.code = '<MÃ ĐƠN THỬ>' AND p."deletedAt" IS NULL
...
| | |
|---|---|
| **2 dòng `Payment`** | 2.000đ và 3.000đ, **mỗi dòng một bé khác nhau** |
| `accountantStatus` | **`PENDING`** cả hai — máy ghi nhận ≠ kế toán xác nhận |
| `ghi_chu` | chứa **`[auto:sepay:…]`** — dây để gỡ gắn tìm lại được |
| phiếu gộp | **`PAID`** |

Và **không giao dịch 5.000đ nào nằm ở UNMATCHED**:

```sql
SELECT count(*) AS con_treo
FROM "BankTransaction"
WHERE amount = 5000 AND status = 'UNMATCHED'
  AND "transferredAt" > now() - interval '2 hours';
```

**Phải ra `0`.**

> ⛔ **DỪNG PILOT nếu:** chỉ có 1 dòng `Payment` · hai dòng cùng một bé · tổng ≠ 5.000đ ·
> phiếu không lên `PAID` · `ghi_chu` không có marker · hoặc giao dịch 5.000đ còn treo.

### C.5 · Dọn sau khi thử — **ĐỂ NGUYÊN**, đừng gỡ

**Khuyến nghị: không dọn gì trong sổ.** Ba lý do:

1. **Gỡ gắn là thêm dòng, không phải bớt dòng.** `goGanTheoCon` sinh **bút toán đảo** cho mỗi
   phần — 5.000đ tiền thử sẽ để lại 2 dòng `PAYMENT` + 2 dòng `ADJUSTMENT` thay vì 2 dòng.
2. **Lượt thử là BẰNG CHỨNG.** Để nguyên thì sáu tháng sau còn tra được "đường này đã chạy
   thật, ngày nào, giao dịch nào".
3. **Không đồng nào sai chỗ.** Tiền đã vào đúng hai bé của đúng đơn thử.

**9.000đ tiền thật (5.000 + 4.000) đã vào tài khoản công ty.** Muốn hoàn cho người đã chuyển:

- chuyển khoản trả lại **như một khoản chi bình thường**, kế toán ghi sổ chi;
- ⛔ **KHÔNG dùng đường "hoàn tiền" của đơn** (`refundPayment`): nó sinh bút toán âm trên
  **đơn thử**, tức làm bẩn đúng cái sổ mà bước 1 muốn giữ sạch làm bằng chứng;
- ⛔ **KHÔNG xoá dòng `Payment` / `PaymentBill` / `BankTransaction` nào.** Sổ tiền không xoá.

**Giao dịch 4.000đ đang treo UNMATCHED:** kế toán vào `/admin/bien-dong-so-du` đánh **Bỏ qua
(IGNORED)** kèm lý do *"thử nghiệm PHIÊN C 21/09 — lệch số có chủ đích"*. Để nguyên UNMATCHED
thì nó nằm mãi trong hàng chờ và người sau sẽ đi tìm một khoản lệch không tồn tại.

**Đơn thử:** để nguyên, hoặc chuyển sang `CANCELLED`. Đừng xoá.

### C.6 · TẮT GẤP — có cách tắt RIÊNG đường phiếu gộp

> **Có**, và nó chính xác hơn tắt cờ. Nhưng chỉ tắt được **một nửa**; nửa kia nói thẳng ở dưới.

#### Nửa CHẶN ĐƯỢC: webhook thôi tự chia tiền

Đưa mọi phiếu **đang mở** về trạng thái không-OPEN. `chiaTheoPhieuGop` kiểm **trạng thái
TRƯỚC số tiền**, nên phiếu không OPEN ⇒ rơi vào `PHIEU_KHONG_MO` ⇒ **không phân bổ**, giao
dịch về UNMATCHED cho người xử.

```sql
-- Phiếu chưa nhận đồng nào → VOID. Phiếu đã nhận một phần → CLOSED (giữ dấu vết).
UPDATE "PaymentBill" pb
SET status = CASE
      WHEN EXISTS (
        SELECT 1 FROM "PaymentBillLine" pbl
        JOIN "PaymentAllocation" pa ON pa."paymentRequestId" = pbl."paymentRequestId"
        WHERE pbl."billId" = pb.id
      ) THEN 'CLOSED'::"PaymentBillStatus"
      ELSE 'VOID'::"PaymentBillStatus"
    END
WHERE pb.status = 'OPEN'
  AND pb."centerId" = (SELECT id FROM "Center" WHERE code = 'CS2');
```

Kiểm lại — phải ra **0**:

```sql
SELECT count(*) AS con_mo FROM "PaymentBill" pb
WHERE pb.status = 'OPEN'
  AND pb."centerId" = (SELECT id FROM "Center" WHERE code = 'CS2');
```

> ✅ **Ưu điểm:** **webhook hiệu lực NGAY** — không chờ cache 5 phút, vì đây là dữ liệu chứ
> không phải cờ; mỗi lượt tiền về đọc lại trạng thái phiếu từ DB. Và **không đụng cờ CS2**,
> nên gắn tay + tách khoản vẫn chạy.
>
> ⚠️ **MÀN HÌNH thì có thể chậm hơn.** SQL tay không gọi `revalidatePath`, nên trang đơn đang
> mở vẫn vẽ phiếu cũ cho tới khi tải lại. Đó chỉ là hiển thị — tiền đã ngừng tự chia từ lệnh
> `UPDATE`. Bảo người đang mở trang **F5**.
>
> ⚠️ **Mã QR đã gửi cho phụ huynh thành mã chết.** Ai quét sẽ chuyển được tiền (ngân hàng
> không biết gì) nhưng tiền rơi vào **UNMATCHED**, kế toán phải xử tay. Trước khi chạy lệnh
> này, **nhắn cho những nhà đã nhận QR**.

#### Nửa KHÔNG chặn được: nút "In QR" vẫn còn

**Không có công tắc riêng cho việc PHÁT phiếu.** Nút "In QR" nằm trong cùng khối *"Công nợ
theo con"*, và khối ấy chỉ có **một** cổng: `billing.flexV1Enabled` của cơ sở.

Muốn chặn phát hành thì phải **tắt cả cờ CS2** (mục 5), và khi đó CS2 mất luôn:

| Mất gì | Hệ quả |
|---|---|
| khối "Công nợ theo con" | biến mất hoàn toàn |
| **gắn khoản cho bé** (đường B) | không dùng được — đây là thứ CS2 đang dùng hằng ngày |
| **tách khoản cho nhiều bé** | không dùng được |
| tạo / huỷ đợt theo con | không dùng được |
| nút huỷ / đóng phiếu gộp | **cũng mất** ⇒ phải đóng phiếu bằng SQL ở trên |

> ⚠️ **Và tắt cờ KHÔNG chặn webhook.** `thuTheoPhieuGop` cố ý **không hỏi cờ** (`grep`
> `laThuTienLinhHoatBat` trong `lib/finance/phieu-gop.ts` ra **0**) — vì mã đã phát là mã bất
> biến, phụ huynh đang giữ tờ QR ấy. Nên tắt cờ mà **không** đóng phiếu thì tiền vẫn tự chia
> như thường.
>
> **Thứ tự tắt gấp đúng: đóng phiếu bằng SQL TRƯỚC, tắt cờ sau (nếu cần).** Làm ngược lại là
> tự khoá mất cái nút mình cần bấm.
