...
// Dùng token brand `primary` của main (tương thích sẵn, không cần accent-mode riêng).
      ...
      <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-center gap-3 sm:gap-4">
          ...
          <div className="min-w-0">
            {overline && (
              <p className="text-xs font-bold uppercase tracking-wider text-white/70">{overline}</p>
            )}
            <h1 className="text-balance text-xl font-bold tracking-tight sm:text-2xl">{title}</h1>
            {subtitle && (
              <p className="mt-0.5 line-clamp-2 text-xs font-medium text-white/80 sm:text-sm">{subtitle}</p>
            )}
          </div>
        </div>
