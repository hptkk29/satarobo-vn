          ...
          {kids.map((c, i) => (
            <div key={c.id} className="flex h-full flex-col gap-3 rounded-2xl border border-border bg-card p-4">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-xs font-medium text-muted-foreground">
                    {[c.courseName, c.className].filter(Boolean).join(" · ") || c.studentCode || "Chưa xếp lớp"}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <Stat value={`${c.attendanceRate}%`} label="Chuyên cần" tone={c.attendanceRate >= 90 ? "success" : "caution"} />
                <Stat value={`${c.pendingHomework}`} label="Bài chờ" tone={c.pendingHomework > 0 ? "primary" : "success"} />
                <Stat value={c.debt > 0 ? "Còn nợ" : "Đủ"} label="Học phí" tone={c.debt > 0 ? "caution" : "success"} />
                <Stat value={nextLabel(c.nextSessionDate)} label="Buổi tiếp theo" tone="primary" />
              </div>
...
function Stat({ value, label, tone }: { value: string; label: string; tone: Tone }) {
  const cls = tone === "success" ? "text-success" : tone === "caution" ? "text-caution" : "text-primary";
  return (
