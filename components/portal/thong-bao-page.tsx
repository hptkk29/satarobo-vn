...
import {
  Bell, MessageSquare, CalendarDays,
  CalendarPlus, Wallet, GraduationCap, ClipboardList, Inbox, Megaphone,
} from "lucide-react";
import type { LucideIcon } from "lucide-react";
import type { FeedItem, FeedCategory, NotificationFeed } from "@/lib/portal/notification-feed";
...
const CAT: Record<FeedCategory, { label: string; icon: LucideIcon; tone: string }> = {
  NHAN_XET: { label: "Nhận xét", icon: MessageSquare, tone: "bg-quiz/10 text-quiz" },
  LICH_HOC: { label: "Lịch học", icon: CalendarDays, tone: "bg-info/10 text-info" },
  HOC_BU: { label: "Học bù", icon: CalendarPlus, tone: "bg-caution/10 text-caution" },
  HOC_PHI: { label: "Học phí", icon: Wallet, tone: "bg-primary/10 text-primary" },
  HOC_BA: { label: "Học bạ", icon: GraduationCap, tone: "bg-accent-soft text-accent" },
  KHAO_SAT: { label: "Khảo sát", icon: ClipboardList, tone: "bg-success/10 text-success" },
  // Notification trung tâm/cơ sở/lớp — nội dung tự do, không gán nhãn "Lịch học".
  THONG_BAO: { label: "Trung tâm", icon: Megaphone, tone: "bg-info/10 text-info" },
};
const ORDER: FeedCategory[] = ["NHAN_XET", "THONG_BAO", "LICH_HOC", "HOC_BU", "HOC_PHI", "HOC_BA", "KHAO_SAT"];
  ...
  return (
    <div className="portal-v2 mx-auto w-full max-w-6xl space-y-6">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 lg:hidden">
        <FilterChip label="Tất cả" count={feed.unreadTotal} active={sel === "ALL"} onClick={() => setSel("ALL")} />
        {ORDER.map((k) => (
          <FilterChip key={k} label={CAT[k].label} count={feed.unreadByCategory[k]} active={sel === k} onClick={() => setSel(k)} />
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[15rem_1fr]">
        ...
        <aside className="hidden space-y-1 rounded-2xl border border-border bg-card p-2 lg:block">
          <FilterRow icon={Inbox} label="Tất cả" count={feed.unreadTotal} active={sel === "ALL"} onClick={() => setSel("ALL")} />
          {ORDER.map((k) => (
            <FilterRow key={k} icon={CAT[k].icon} label={CAT[k].label} count={feed.unreadByCategory[k]} active={sel === k} onClick={() => setSel(k)} />
          ))}
        </aside>
        ...
        <div className="space-y-6">
          <section className="space-y-3">
            <h2 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Mới ({unread.length})</h2>
            {unread.length === 0 ? (
              <p className="rounded-2xl border border-border bg-card p-6 text-center text-sm text-muted-foreground">Không có thông báo mới.</p>
            ) : (
          ...
          {read.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Trước đó</h2>
...
function FilterChip({ label, count, active, onClick }: { label: string; count: number; active: boolean; onClick: () => void }) {
  return (
    ...
    >
      {label}
      {count > 0 && <span className={`rounded-full px-1.5 py-0.5 text-[11px] font-bold ${active ? "bg-accent text-white" : "bg-primary/10 text-primary"}`}>{count}</span>}
    </button>
  );
}

function FilterRow({ icon: Icon, label, count, active, onClick }: { icon: LucideIcon; label: string; count: number; active: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
...
function Card({ n, unread }: { n: FeedItem; unread?: boolean }) {
  ...
  const inner = (
    <div className="flex items-start gap-3">
      <span className={`grid size-9 shrink-0 place-items-center rounded-xl ${c.tone}`}><Icon className="size-4" /></span>
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-2">
          <h3 className={`text-sm ${unread ? "font-bold text-foreground" : "font-semibold text-foreground/90"}`}>{n.title}</h3>
          {n.childName && <span className="shrink-0 rounded-md bg-accent-soft px-2 py-0.5 text-xs font-semibold text-accent">{n.childName.split(" ").slice(-2).join(" ")}</span>}
        </div>
        <p className="mt-0.5 text-sm text-muted-foreground">{n.body}</p>
        <p className="mt-1.5 text-xs font-medium uppercase tracking-wide text-muted-foreground/70">{c.label} · <span className="normal-case">{timeAgo(n.at)}</span></p>
      </div>
    </div>
  );
  ...
  if (href) {
    return (
      <Link href={href} className={`${cls} transition-colors hover:border-accent/60`}>
        {inner}
      </Link>
    );
  }
