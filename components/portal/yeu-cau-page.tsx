...
import { CalendarPlus, TriangleAlert, Clock, CircleCheck, CalendarSearch, History, ShieldCheck, Send } from "lucide-react";
  ...
  return (
    <div className="portal-v2 mx-auto w-full max-w-6xl space-y-6">
      ...
      <PageHero
        ...
        metric={<HeroMetric label="Cần học bù" value={String(data.needCount)} />}
      />

      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <Stat icon={TriangleAlert} tone="caution" value={data.needCount} label="Buổi cần học bù" />
        ...
        <Stat icon={Clock} tone="info" value={data.pendingCount} label="Đã xếp buổi bù" />
        <Stat icon={CircleCheck} tone="success" value={data.doneCount} label="Đã học bù" />
      </div>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_18rem]">
        <div className="space-y-6">
          <section className="space-y-3">
            <h2 className="text-sm font-bold uppercase tracking-wider text-foreground">Buổi cần học bù</h2>
            {data.needList.length === 0 ? (
              <div className="flex items-center gap-3 rounded-2xl border border-success/30 bg-success/5 p-4 text-sm font-semibold text-success">
                <CircleCheck className="size-5 shrink-0" /> Không có buổi nào cần học bù.
              </div>
            ) : (
              data.needList.map((m) => (
                <div key={m.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-caution/30 bg-caution/5 p-4">
                  <div className="flex items-start gap-3">
                    <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-caution/15 text-caution"><TriangleAlert className="size-5" /></span>
                    <div>
                      <p className="text-sm font-bold text-foreground">{m.lessonTitle}</p>
                      <p className="mt-0.5 text-xs font-medium text-muted-foreground">Vắng ngày {dmy(m.missedDate)} · {m.reason}</p>
                    </div>
                  </div>
          ...
          <section className="space-y-3">
            <h2 className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-foreground"><History className="size-4 text-primary" /> Lịch sử yêu cầu học bù</h2>
            {data.history.length === 0 ? (
              <p className="rounded-2xl border border-border bg-card p-6 text-center text-sm text-muted-foreground">Chưa có lịch sử học bù.</p>
            ) : (
              <div className="space-y-3">
                {data.history.map((m) => (
                  <div key={m.id} className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-border bg-card p-4">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-bold text-foreground">{m.lessonTitle}</p>
                      <p className="mt-0.5 text-xs font-medium text-muted-foreground">
                        Lớp {m.className.replace(/^Lớp\s+/i, "")}{m.centerName ? ` · ${m.centerName}` : ""} · {dmy(m.missedDate)}
                      </p>
                    </div>
          ...
          <section className="space-y-3">
            <h2 className="text-sm font-bold uppercase tracking-wider text-foreground">Yêu cầu đã gửi</h2>
            {requests.length === 0 ? (
                ...
                {requests.map((r) => (
                  <div key={r.id} className="rounded-2xl border border-border bg-card p-4">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <span className="text-sm font-bold text-foreground">{REQUEST_TYPE_LABEL[r.type]}</span>
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${REQUEST_STATUS_BADGE[r.status]}`}>
                        {REQUEST_STATUS_LABEL[r.status]}
                      </span>
                    </div>
                    <p className="mt-1 whitespace-pre-wrap text-sm text-muted-foreground">{r.content}</p>
                    {r.preferredDate && (
                      <p className="mt-1 text-xs text-muted-foreground">Ngày: {dmy(r.preferredDate)}</p>
                    )}
                    {r.response && (
                      <p className="mt-2 rounded-lg bg-muted p-2 text-sm text-muted-foreground">Phản hồi: {r.response}</p>
                    )}
                    <div className="mt-2 flex items-center justify-between">
                      <span className="text-xs text-muted-foreground">{dmyVN(r.createdAt)}</span>
        ...
        <div className="space-y-3 rounded-2xl border border-border bg-card p-4">
          <p className="flex items-center gap-2 text-sm font-bold text-foreground"><ShieldCheck className="size-4 text-primary" /> Chính sách học bù</p>
          <ul className="space-y-2 text-xs font-medium text-muted-foreground">
            <li>• Học bù đúng nội dung buổi đã vắng.</li>
            <li>• Cho phép học bù <b className="text-foreground">liên cơ sở</b> (CS1 ↔ CS2).</li>
            <li>• Ưu tiên hiển thị lớp tại cơ sở của con trước.</li>
            <li>• Yêu cầu cần CRM/Quản lý duyệt trước khi học.</li>
          </ul>
          {data.centerName && (
            <p className="border-t border-border pt-3 text-xs text-muted-foreground">
              Cơ sở của {studentName.split(" ").slice(-1)[0]}: <b className="text-foreground">{data.centerName}</b>
            </p>
          )}
        </div>
      </div>
...
function Stat({ icon: Icon, tone, value, label }: { icon: typeof Clock; tone: "caution" | "info" | "success"; value: number; label: string }) {
  const box = tone === "caution" ? "bg-caution/10 text-caution" : tone === "info" ? "bg-info/10 text-info" : "bg-success/10 text-success";
  const val = tone === "caution" ? "text-caution" : tone === "info" ? "text-info" : "text-success";
  return (
...
function Badge({ status }: { status: MakeupItem["status"] }) {
  ...
  return <span className={`shrink-0 rounded-md px-2.5 py-1 text-xs font-bold ${s.cls}`}>{s.label}</span>;
}
