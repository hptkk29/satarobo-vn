...
import { roadmap5Years, type RoadmapCourse, type RoadmapModule } from "./_data/roadmap-5-years";
import { examRoadmap, type ExamRoadmapItem } from "./_data/exam-roadmap";
...
const courseMeta: Record<string, CourseMetaEntry> = {
...
const featuredProjectsByCourse: Record<string, [string, string, string][]> = {
  Sata3: [
    ["Bàn Tay Ma Thuật", "/laptrinhrobot/sata3_1.jpg", "Lắp ráp chuyển động cơ bản và quan sát cơ cấu."],
    ["Siêu Xe Bứt Phá", "/laptrinhrobot/sata3_2.jpg", "Tạo mô hình xe robot đầu tiên."],
    ["Vũ Công Robot", "/laptrinhrobot/sata3_3.jpg", "Điều khiển robot bằng chuỗi lệnh trực quan."],
    ["Thần Long Trỗi Dậy", "/laptrinhrobot/sata3_4.jpg", "Khám phá truyền động và cân bằng mô hình."],
    ["Siêu Xe Chuyên Dụng", "/laptrinhrobot/sata3_5.jpg", "Tối ưu cấu trúc xe theo nhiệm vụ."],
  ],
  Sata4: [
    ["Xe robot di chuyển", "/laptrinhrobot/sata4_1.jpg", "Lập trình robot di chuyển theo nhiệm vụ."],
    ["Cảm biến dò line", "/laptrinhrobot/sata4_2.jpg", "Nhận biết vạch và tín hiệu cảm biến."],
    ["Xe robot tự hành", "/laptrinhrobot/sata4_3.jpg", "Kết hợp cảm biến để robot tự vận hành."],
    ["Dò line nâng cao", "/laptrinhrobot/sata4_4.jpg", "Tinh chỉnh đường chạy và tốc độ."],
    ["Thi đấu nội bộ", "/laptrinhrobot/sata4_5.jpg", "Rèn phản xạ chiến thuật trong sa bàn."],
  ],
  Sata5: [
    ["Máy đập bóng cơ", "/laptrinhrobot/sata5_1.jpg", "Thiết kế cơ cấu chuyển động có lực."],
    ["Cổng quét an ninh", "/laptrinhrobot/sata5_2.jpg", "Ứng dụng cảm biến vào mô hình tự động."],
    ["Cửa nhà thông minh", "/laptrinhrobot/sata5_3.jpg", "Mô phỏng smart-home bằng robot."],
    ["Xe bám mục tiêu", "/laptrinhrobot/sata5_4.jpg", "Robot phản hồi theo tín hiệu môi trường."],
    ["Hệ thống phân loại", "/laptrinhrobot/sata5_5.jpg", "Phân loại vật thể theo quy trình tự động."],
  ],
  Sata6: [
    ["Cảm biến dò line", "/laptrinhrobot/sata6_1.jpg", "Xây nền xử lý cảm biến cho thi đấu."],
    ["Dò line chuyên sâu PID", "/laptrinhrobot/sata6_2.jpg", "Giữ robot ổn định trên đường chạy."],
    ["Tối ưu chương trình", "/laptrinhrobot/sata6_3.jpg", "Rút ngắn thời gian và giảm lỗi vận hành."],
    ["Demo thi đấu", "/laptrinhrobot/sata6_4.jpg", "Thử nghiệm chiến thuật trên sa bàn."],
    ["Thi đấu nội bộ", "/laptrinhrobot/sata6_5.jpg", "Luyện áp lực và phối hợp đội thi."],
  ],
  Sata7: [
    ["AI ra lệnh robot", "/laptrinhrobot/sata7_1.jpg", "Điều khiển robot bằng tín hiệu thông minh."],
    ["AI nhận diện màu", "/laptrinhrobot/sata7_2.jpg", "Ứng dụng thị giác máy tính cơ bản."],
    ["AI robot tự hành", "/laptrinhrobot/sata7_3.jpg", "Robot xử lý môi trường và ra quyết định."],
    ["AI quét mã QR", "/laptrinhrobot/sata7_4.jpg", "Nhận diện dữ liệu bằng camera."],
    ["AI nhận diện mặt", "/laptrinhrobot/sata7_5.jpg", "Mô phỏng bài toán nhận diện trong thực tế."],
  ],
};
...
interface ExamCourseCardProps {
  item: ExamRoadmapItem;
  course: Course;
  isOpen: boolean;
  onToggle: () => void;
}

function ExamCourseCard({ item, course, isOpen, onToggle }: ExamCourseCardProps) {
  return (
    <article className="rounded-3xl border border-gray-100 bg-white p-5 shadow-card sm:p-6">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <span className="inline-flex rounded-full bg-soft-purple px-3 py-1 text-xs font-black text-primary-purple">{course.id}</span>
...
function FocusCourseBox({ item, course, isOpen, onToggle }: FocusCourseBoxProps) {
  ...
  return (
    ...
    >
      <div className={`-mx-5 -mt-5 mb-5 rounded-t-3xl px-5 py-3 text-center text-xs font-black uppercase tracking-widest text-white sm:-mx-6 sm:-mt-6 sm:px-6 ${isCombo ? "bg-primary-orange" : "bg-primary-purple"}`}>
      ...
      <div className="mb-4 flex items-start gap-3">
        ...
        <div>
          <span className="mb-2 inline-flex rounded-full bg-white px-3 py-1 text-[11px] font-black text-primary-orange shadow-sm">{course.badge}</span>
...
function FeaturedProjects({ productCode, onOpen, paused = false }: { productCode: string; onOpen: (payload: LightboxPayload) => void; paused?: boolean }) {
      ...
      <div className="mt-2 grid grid-cols-5 gap-1.5">
        {projects.map((item, index) => (
          <button
            ...
            type="button"
            onClick={() => go(index)}
            className={`aspect-square overflow-hidden rounded-lg border-2 transition ${index === active ? "border-primary-orange shadow-sm" : "border-transparent opacity-70 hover:opacity-100"}`}
...
function ProjectLightbox({ projects, startIdx, onClose }: { projects?: FeaturedProject[]; startIdx?: number; onClose: () => void }) {
  ...
  if (!total || !project || !projects) return null;

  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 p-4" onClick={onClose}>
      <div className="flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        <div className="flex flex-shrink-0 items-center justify-between gap-3 border-b border-gray-100 px-5 py-3.5">
          <div className="min-w-0">
            <div className="mb-0.5 text-[11px] font-bold uppercase tracking-wider text-primary-orange">Dự án {idx + 1} / {total}</div>
          ...
          <button type="button" onClick={onClose} className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-gray-100 transition hover:bg-gray-200" aria-label="Đóng ảnh">
            <X className="h-4 w-4" />
          </button>
        </div>

        <div className="relative flex-1 overflow-hidden bg-gray-950">
          <img key={project.image} src={project.image} alt={project.title} className="mx-auto block max-h-[62vh] w-full object-contain" />
          <button type="button" onClick={(e) => { e.stopPropagation(); prev(); }} className="absolute left-3 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-black/50 text-white shadow-lg backdrop-blur-sm transition hover:bg-black/75 active:scale-95" aria-label="Ảnh trước">
            <ChevronLeft className="h-6 w-6" />
          </button>
          <button type="button" onClick={(e) => { e.stopPropagation(); next(); }} className="absolute right-3 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-black/50 text-white shadow-lg backdrop-blur-sm transition hover:bg-black/75 active:scale-95" aria-label="Ảnh tiếp">
          ...
          <div className="flex items-center justify-center gap-1.5 overflow-x-auto">
            {projects.map((item, i) => (
              <button
                ...
                type="button"
                onClick={() => setIdx(i)}
                className={`h-12 w-12 flex-shrink-0 overflow-hidden rounded-xl border-2 transition ${i === idx ? "border-primary-orange shadow-sm shadow-primary-orange/30" : "border-transparent opacity-50 hover:opacity-90"}`}
...
const countModuleProjects = (module: RoadmapModule): number => {
...
export default function Roadmap5Years() {
  ...
  const [lightboxState, setLightboxState] = useState<LightboxPayload | null>(null);
  const [mobileDropdownOpen, setMobileDropdownOpen] = useState(false);
  const mobileDropdownRef = useRef<HTMLDivElement>(null);

  const currentYear: RoadmapCourse = roadmap5Years[yearIdx];
  const currentModule: RoadmapModule = currentYear.modules[moduleIdx];
  ...
  const examItems = useMemo(
    () =>
      examRoadmap
        ...
        .filter((item): item is ExamRoadmapItem & { course: Course } => Boolean(item.course)),
    [],
  );
  ...
  useEffect(() => {
    ...
    if (stored && Number.isInteger(stored.yearIndex)) {
      setActiveTrack("deep");
      setYearIdx(stored.yearIndex as number);
      setModuleIdx(0);
    }

    const handleCourseSelected = (event: Event) => {
      const detail = (event as CustomEvent<CourseSelectionPayload>).detail;
      const productCode = detail?.productCode;
      const yearIndex = detail?.yearIndex;

      if (Number.isInteger(yearIndex) && (yearIndex as number) >= 0 && (yearIndex as number) < roadmap5Years.length) {
        setActiveTrack("deep");
        setYearIdx(yearIndex as number);
        setModuleIdx(0);
        return;
      }
  ...
  const tabs: { id: "exam" | "deep"; title: string; subtitle: string }[] = [
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`rounded-xl border-2 px-4 py-3 text-left transition ${active ? "border-primary-orange bg-gradient-orange-purple text-white shadow-orange-glow" : "border-transparent bg-white text-text-dark hover:border-primary-orange/40"}`}
              >
                ...
                <ExamCourseCard
                  ...
                  onToggle={() =>
                    setOpenExamIds((current) => {
                      if (isTabletUp) return current.length ? [] : shortItems.map((c) => c.id);
                ...
                <FocusCourseBox
                  ...
                  onToggle={() =>
                    setOpenFocusIds((current) => {
                      if (isTabletUp) return current.length ? [] : focusItems.map((c) => c.id);
                      return current.includes(item.id) ? current.filter((id) => id !== item.id) : [...current, item.id];
                    })
                  }
                />
          ...
          <div className="animate-fade-in">
            ...
            <div className="relative mb-6 md:hidden" ref={mobileDropdownRef}>
              <button
                ...
                className={`flex w-full items-center justify-between gap-3 rounded-2xl border-2 p-3.5 text-left transition-all ${mobileDropdownOpen ? "border-primary-orange bg-soft-cream shadow-orange-glow" : "border-primary-orange/40 bg-soft-cream hover:border-primary-orange"}`}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-[11px] font-black uppercase tracking-wider text-primary-orange">Đang xem · Sata{currentYear.year + 2}</div>
              ...
              {mobileDropdownOpen && (
                <div role="listbox" className="absolute left-0 right-0 top-full z-30 mt-2 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl animate-fade-in">
                  ...
                  {roadmap5Years.map((year, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex w-full items-center gap-3 px-4 py-3.5 text-left transition-colors ${active ? "bg-soft-cream" : "hover:bg-gray-50"} ${i < roadmap5Years.length - 1 ? "border-b border-gray-100" : ""}`}
                      >
                ...
                {roadmap5Years.map((year, idx) => {
                  ...
                  return (
                    <button
                      key={year.productCode}
                      type="button"
                      onClick={() => { setYearIdx(idx); setModuleIdx(0); }}
                      className={`min-w-[170px] rounded-2xl border-2 p-3 text-left transition ${idx === yearIdx ? "border-primary-orange bg-soft-cream shadow-orange-glow" : "border-gray-200 bg-white hover:border-primary-orange/50"}`}
                    >
                      <div className="mb-2 flex items-center gap-2">
                        <span className={`flex h-11 w-11 items-center justify-center rounded-2xl border text-lg ${idx === yearIdx ? meta.activeWrap : meta.iconWrap}`}>
                          <Icon className="h-5 w-5" />
                        </span>
                        <span className="text-xs font-black uppercase text-primary-orange">Sata{year.year + 2}</span>
                      </div>
            ...
            <div className="mb-8 rounded-3xl border-2 border-primary-orange/20 bg-gradient-to-br from-white via-soft-cream to-soft-purple/60 p-5 shadow-card sm:p-7 lg:p-8">
              <div className="grid gap-6 lg:grid-cols-12">
                <div className="lg:col-span-8">
                  ...
                  <h3 className="mb-3 text-2xl font-black leading-tight text-text-dark sm:text-3xl">{currentYear.productName}</h3>
                ...
                <aside className="h-fit rounded-3xl border border-primary-purple/20 bg-white/95 p-5 shadow-card lg:col-span-4">
                  ...
                  <FeaturedProjects productCode={currentYear.productCode} onOpen={setLightboxState} paused={Boolean(lightboxState)} />
                </aside>
              </div>
            </div>
            ...
            <div className="mb-6 flex items-center justify-between px-2">
              {currentYear.modules.map((mod, idx) => (
                <div key={mod.id} className="flex flex-1 items-center last:flex-initial">
                  <button type="button" onClick={() => setModuleIdx(idx)} className={`flex flex-col items-center transition ${idx === moduleIdx ? "scale-110" : "opacity-65 hover:opacity-100"}`}>
                    <div className={`flex h-10 w-10 items-center justify-center rounded-full text-sm font-black sm:h-12 sm:w-12 ${idx === moduleIdx ? "bg-primary-orange text-white shadow-orange-glow" : idx < moduleIdx ? "bg-success text-white" : "bg-gray-200 text-text-muted"}`}>
                      {idx < moduleIdx ? "✓" : idx + 1}
                    </div>
              ...
              <div className="hidden space-y-3 lg:col-span-4 lg:block">
                {currentYear.modules.map((mod, idx) => (
                  <button
                    ...
                    className={`block w-full rounded-xl border-2 p-4 text-left transition ${idx === moduleIdx ? "border-primary-orange bg-soft-cream shadow-orange-glow" : "border-gray-200 bg-white hover:border-primary-orange/50"}`}
                  >
              ...
              <div className="lg:col-span-8" key={`${yearIdx}-${moduleIdx}`}>
                <div className="card-base border-2 border-primary-orange/20 p-5 sm:p-7">
                  ...
                  <div className="mt-6 flex items-center justify-between gap-2 border-t border-gray-200 pt-5">
                    <button type="button" onClick={goPrevModule} disabled={yearIdx === 0 && moduleIdx === 0} className="inline-flex items-center gap-1.5 rounded-xl border-2 border-gray-200 bg-soft-cream px-4 py-2.5 text-sm font-bold text-text-dark transition hover:border-primary-orange hover:text-primary-orange disabled:opacity-30">
                      <ChevronLeft className="h-4 w-4" />
                      Trước
                    </button>
                    <span className="px-2 text-xs font-semibold text-text-muted">HP {moduleIdx + 1} / 4</span>
                    <button type="button" onClick={goNextModule} disabled={yearIdx === 4 && moduleIdx === 3} className="inline-flex items-center gap-1.5 rounded-xl bg-primary-orange px-4 py-2.5 text-sm font-bold text-white shadow-orange-glow transition hover:bg-primary-orange-dark disabled:opacity-30">
                      Tiếp theo
