  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="relative inline-flex h-9 w-9 items-center justify-center rounded-lg text-neutral-500 transition-colors hover:bg-neutral-100 hover:text-neutral-900"
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 w-80 rounded-xl border border-neutral-200 bg-white shadow-lg">
          ...
          <div className="max-h-96 overflow-y-auto">
            {items.length === 0 ? (
              <p className="px-3 py-8 text-center text-sm text-neutral-400">Không có thông báo.</p>
            ) : (
              items.map((i) => (
                <button
                  key={i.id}
