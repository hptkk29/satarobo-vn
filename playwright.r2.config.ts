/**
 * Playwright config cho Phase R1 (tests/e2e/r2) — Postgres LOCAL (.env.test).
 * Cùng cơ chế playwright.a0.config.ts; tái dùng global-setup (migrate deploy).
 *   pnpm db:test:up && pnpm test:e2e:r2
 */
...
export default defineConfig({
  ...
  projects: [{ name: "r2", use: { ...devices["Desktop Chrome"] } }],

  webServer: process.env.R2_SKIP_WEBSERVER
    ? undefined
    : {
