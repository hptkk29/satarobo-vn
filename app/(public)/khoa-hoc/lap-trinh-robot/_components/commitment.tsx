...
import { commitments, type Commitment } from '../_data/commitments'
...
function SlideCarousel({ items }: { items: Commitment[] }) {
        ...
        >
          {items.map((item, i) => (
            <div key={i} className="min-w-full">
              <CommitmentCard commitment={item} index={i} />
            </div>
          ))}
        </div>
      </div>
          ...
          {items.map((_, i) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                i === idx ? 'w-6 bg-primary-purple' : 'w-2 bg-gray-300 hover:bg-gray-400'
              }`}
    ...
    <section id="commitment" className="section-padding bg-soft-cream">
      <div className="container-site">
        ...
        <div className="mb-10 md:hidden">
          <SlideCarousel items={commitments} />
        </div>
        ...
        <div className="mb-10 hidden gap-4 sm:gap-5 md:grid md:grid-cols-2 lg:grid-cols-3">
          {commitments.map((commitment, index) => (
            <div key={commitment.id} className="h-full transition hover:-translate-y-1 hover:shadow-card-hover">
              <CommitmentCard commitment={commitment} index={index} />
            </div>
          ))}
        </div>
        ...
        <div className="mx-auto max-w-3xl rounded-2xl border-2 border-primary-purple/20 bg-white p-6 text-center shadow-md sm:p-8">
          ...
          <p className="mb-5 text-sm text-text-muted sm:text-base">
            Sata Robo ưu tiên sự minh bạch: quyền lợi học viên, các mốc thuyết trình và điều kiện cam kết
            đều được nói rõ ngay từ đầu.
          </p>
          <div className="flex flex-col justify-center gap-3 sm:flex-row">
            <a href="#registration-form" className="btn-primary">
