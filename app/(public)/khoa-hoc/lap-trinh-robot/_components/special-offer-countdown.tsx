...
const iconMap: Record<string, React.ElementType> = {
  CreditCard, Flame, Gift, Sparkles, Trophy, Users,
}

type PromoItem = (typeof promotions.primary)[number] | (typeof promotions.secondary)[number]
type PromoWithType = PromoItem & { _type: 'primary' | 'secondary' }
...
function SlideCarousel({
  ...
  items: PromoWithType[]
  renderItem: (item: PromoWithType, i: number) => React.ReactNode
}) {
  ...
  const onTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(delta) > 40) { if (delta > 0) { next() } else { prev() }; pauseAndResume() }
    touchX.current = null
  }
          ...
          {items.map((_, i) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                i === idx ? 'w-6 bg-primary-orange' : 'w-2 bg-gray-300 hover:bg-gray-400'
              }`}
...
function PrimaryCard({ promo }: { promo: (typeof promotions.primary)[number] }) {
...
function SecondaryCard({ promo }: { promo: (typeof promotions.secondary)[number] }) {
...
export function SpecialOfferCountdown() {
  ...
  const allPromos: PromoWithType[] = [
    ...promotions.primary.map(p => ({ ...p, _type: 'primary' as const })),
    ...promotions.secondary.map(p => ({ ...p, _type: 'secondary' as const })),
  ]

  const renderCard = (promo: PromoWithType) =>
    promo._type === 'primary'
      ? <PrimaryCard promo={promo as (typeof promotions.primary)[number]} />
      : <SecondaryCard promo={promo as (typeof promotions.secondary)[number]} />
    ...
    <section id="special-offer" className="relative overflow-hidden bg-gradient-to-br from-soft-cream via-white to-soft-purple/50 py-12 sm:py-16 lg:py-20">
      ...
      <div className="container-site relative">
        ...
        <div className="mx-auto mb-7 grid max-w-xl grid-cols-4 gap-2 sm:mb-9 sm:gap-3">
          {countdownItems.map(item => (
            <div key={item.label} className="rounded-2xl border border-primary-purple/10 bg-white/90 p-3 text-center shadow-card sm:p-4">
              <div
                className="font-black tabular-nums leading-none text-primary-purple"
        ...
        <div className="flex flex-col justify-center gap-3 sm:flex-row">
          <a href="#registration-form" className="btn-primary w-full text-center sm:w-auto">
            Đăng ký nhận ưu đãi
            <ArrowRight className="h-5 w-5" />
          </a>
          <a href="#roadmap" className="btn-outline w-full text-center sm:w-auto">
            Tư vấn khóa phù hợp
          </a>
        </div>
      </div>
