'use client'

import Image from 'next/image'
import { CompetitionRoadmap } from './competition-roadmap'
import { trackAndOpen } from '../_utils/track-cta'

export function Hero() {
  return (
    <section
      id="hero"
      aria-labelledby="hero-heading"
      className="overflow-hidden w-full"
      style={{
        background: 'linear-gradient(160deg, #fff9f5 0%, #ffffff 50%, #f5f3ff 100%)',
      }}
    >
      {/* 2-col grid */}
      <div className="container-site grid md:grid-cols-2 gap-10 sm:gap-12 items-center py-10 sm:py-12 pb-6">
        {/* Left */}
        <div className="flex flex-col gap-5">
          <span
            role="note"
            className="inline-flex items-center gap-2 text-white text-xs sm:text-sm font-bold px-4 py-2 rounded-full w-fit"
            style={{
              background: 'linear-gradient(135deg, #F97316, #FBBF24)',
              boxShadow: '0 4px 16px rgba(249,115,22,0.35)',
              animation: 'pulse 2s ease-in-out infinite',
            }}
          >
            🚨 Vòng loại RBT2026 đã mở — Cả nước đang luyện
          </span>

          <h1
            id="hero-heading"
            className="text-3xl sm:text-4xl lg:text-5xl font-black leading-tight text-text-dark"
          >
            Con bạn sẵn sàng
            <br />
            chinh phục{' '}
            <span className="text-gradient-orange-purple">Robotics 2026?</span>
          </h1>

          <p className="text-base sm:text-lg text-text-muted leading-relaxed">
            Khoá Luyện Thi <strong>RoboSim</strong> của Sata Robo được tạo ra để{' '}
            <strong>đưa con vào TOP</strong> — pass vòng loại{' '}
            <strong className="text-primary-orange">26/07/2026</strong>.
          </p>

          <div className="border-l-4 border-primary-orange bg-orange-50 pl-4 pr-4 py-3 rounded-r-2xl">
            <p className="text-xs font-extrabold text-gray-400 uppercase tracking-wider mb-1">
              Mục tiêu duy nhất của khoá học
            </p>
            <p className="text-lg sm:text-xl font-black text-primary-orange uppercase tracking-wide">
              Đưa con vào TOP. Pass vòng loại.
            </p>
          </div>

          <div className="flex gap-3 flex-wrap">
            <button
              onClick={() => trackAndOpen('R1', 'hero_R1')}
              aria-label="Đăng ký Bảng R1 cho con học Tiểu học — 490.000đ"
              className="flex-1 min-w-[160px] rounded-2xl py-4 px-5 font-extrabold text-base text-white text-center transition hover:opacity-90 hover:-translate-y-0.5"
              style={{
                background: 'linear-gradient(135deg, #F7941D 0%, #FFAD4D 100%)',
                boxShadow: '0 8px 24px rgba(247,148,29,0.40)',
              }}
            >
              🟦 Tiểu học → Bảng R1
              <span className="block text-sm font-bold opacity-90 mt-0.5">490.000đ</span>
            </button>
            <button
              onClick={() => trackAndOpen('R2', 'hero_R2')}
              aria-label="Đăng ký Bảng R2 cho con học THCS — 490.000đ"
              className="flex-1 min-w-[160px] rounded-2xl py-4 px-5 font-extrabold text-base text-white text-center transition hover:opacity-90 hover:-translate-y-0.5"
              style={{
                background: 'linear-gradient(135deg, #5b21b6 0%, #8b5cf6 100%)',
                boxShadow: '0 8px 24px rgba(124,58,237,0.35)',
              }}
            >
              🟪 THCS → Bảng R2
              <span className="block text-sm font-bold opacity-90 mt-0.5">490.000đ</span>
            </button>
          </div>

          <div className="flex flex-col gap-1.5 text-sm text-text-muted">
            <span>⏰ Ưu đãi GIẢM đến 45%.</span>
            <span>⭐ 100% Học viên tại SataRobo đã <strong>PASS</strong> qua vòng loại.</span>
          </div>
        </div>

        {/* Right: mascot */}
        <div className="relative flex items-end justify-center min-h-[320px] sm:min-h-[420px]">
          <div
            className="absolute inset-0 rounded-full pointer-events-none"
            style={{
              background: 'radial-gradient(ellipse at 55% 55%, #ffd6a5 0%, #e9d5ff 55%, transparent 80%)',
              filter: 'blur(48px)',
              opacity: 0.7,
            }}
          />
          <Image
            src="/images/courses/luyen-thi-robosim/linh-vat.png"
            alt="Linh vật Sata Robo"
            width={400}
            height={400}
            className="relative z-10 w-full max-w-[340px] sm:max-w-[400px] object-contain"
            style={{ filter: 'drop-shadow(0 24px 48px rgba(124,58,237,0.28))' }}
            priority
          />

          {/* Floating card — exam date */}
          <div
            className="absolute top-[20%] -left-4 z-10 bg-white rounded-2xl shadow-xl border border-orange-100 px-4 py-2.5"
            style={{ animation: 'float 3.5s ease-in-out infinite' }}
          >
            <div className="text-xs text-gray-400 font-semibold">📅 Vòng loại</div>
            <div className="text-base font-extrabold text-primary-orange">26/07/2026</div>
          </div>

          {/* Floating card — achievement */}
          <div
            className="absolute bottom-[18%] -right-4 z-10 bg-white rounded-2xl shadow-xl border border-purple-100 px-4 py-2.5"
            style={{ animation: 'float 4s ease-in-out infinite 0.8s' }}
          >
            <div className="text-xs text-gray-400 font-semibold">🏆 Học viên Sata Robo</div>
            <div className="text-sm font-extrabold text-primary-purple">Chinh phục đấu trường Quốc tế</div>
          </div>
        </div>
      </div>

      {/* Competition roadmap */}
      <div className="container-site pb-12">
        <p className="text-center text-sm text-text-muted font-medium mb-5">
          Hành trình từ vòng loại đến chung kết thế giới — con bố mẹ có thể đi hết:
        </p>
        <CompetitionRoadmap />
        <p className="text-center font-bold text-text-dark mt-5 text-sm sm:text-base">
          Mọi đứa trẻ đều mơ tới chung kết — nhưng tất cả đều bắt đầu từ{' '}
          <span className="text-primary-orange">vòng loại 26/07/2026.</span>
        </p>
      </div>
    </section>
  )
}
