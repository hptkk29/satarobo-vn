...
// Trình soạn đề GV tự tạo — Dialog CONTROLLED (parent giữ open + nút mở, không
// DialogTrigger). Nhiều câu; mỗi câu: Trắc nghiệm (N lựa chọn + đánh dấu đáp án đúng)
// hoặc Tự luận (đáp án mẫu tuỳ chọn). Lưu → createOwnTemplateAction. Cam-only.
...
export function CreateAssignmentDialog({
  ...
  /** Gọi sau khi lưu xong (vd router.refresh để danh sách/nguồn cập nhật). */
    ...
    <Dialog open={open} onOpenChange={(v) => (v ? onOpenChange(true) : close())}>
      <DialogContent className="flex max-h-[88vh] w-[calc(100vw-2rem)] max-w-2xl flex-col gap-0 p-0">
        <DialogHeader className="border-b border-border px-5 py-4">
          ...
          <DialogDescription>
            Soạn đề trắc nghiệm hoặc tự luận. Bài lưu vào Kho của bạn, giao lại cho lớp
            mình phụ trách ở trang Bài tập.
          </DialogDescription>
        </DialogHeader>
