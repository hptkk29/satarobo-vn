...
export default async function BulkConvertPage() {
  ...
  const recorded = leadIds.length
    ? await sdb.payment.findMany({
        where: { saleStatus: 'RECORDED', deletedAt: null, order: { leadId: { in: leadIds } } },
        select: { order: { select: { leadId: true } } },
      })
    : []
  const hasPaymentLeadIds = [
    ...new Set(recorded.map((p) => p.order?.leadId).filter((id): id is string => Boolean(id))),
  ]

  const classes = await sdb.class.findMany({
    where: { deletedAt: null, status: { in: ['PLANNED', 'RECRUITING', 'ACTIVE'] } },
    ...
    <div className="p-6">
      ...
      <div className="mb-6 border-b border-border pb-4">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          ...
          (lùi ngày) để công nợ phản ánh đúng; bỏ trống nếu chưa thu được thông tin tiền.
        </p>
      </div>
