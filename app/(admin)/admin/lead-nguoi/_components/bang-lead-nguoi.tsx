...
import { locLeadNguoiAction, phanBoLeadNguoiAction } from "../_actions";
...
export function BangLeadNguoi({
  ...
  const [nguong, setNguong] = useState(String(nguongBanDau));
  const [centerId, setCenterId] = useState("");
  const [tong, setTong] = useState(tongBanDau);
  const [dong, setDong] = useState<Dong[]>(dongBanDau);
  ...
  const soTrang = Math.max(1, Math.ceil(dong.length / SO_DONG_MOI_TRANG));
  const trangHt = Math.min(Math.max(1, trang), soTrang);
  const dsTrang = dong.slice((trangHt - 1) * SO_DONG_MOI_TRANG, trangHt * SO_DONG_MOI_TRANG);
  ...
  const chonHetTrang = () =>
    setChon((p) => {
      const n = new Set(p);
      const duHet = dsTrang.every((d) => n.has(d.id));
      for (const d of dsTrang) {
  ...
  const locLai = () =>
    batDau(async () => {
      const r = await locLeadNguoiAction({ nguongNgay: nguong, centerId });
      if (!r.ok) {
        toast.error(r.error ?? "Không lọc được");
        return;
      }
      setTong(r.tong ?? 0);
      setDong(r.dong ?? []);
      setChon(new Set());
      setTrang(1);
      // Ngưỡng đã được server kéo về khoảng hợp lệ — hiện lại con số THẬT đang dùng, đừng để
      // ô nhập nói một đằng mà danh sách lọc theo một nẻo.
      if (r.nguongNgay != null) setNguong(String(r.nguongNgay));
    });
  ...
  const phanBo = () =>
    batDau(async () => {
      ...
      const r2 = await locLeadNguoiAction({ nguongNgay: nguong, centerId });
      if (r2.ok) {
        setTong(r2.tong ?? 0);
        setDong(r2.dong ?? []);
      }
  ...
  const sanSang = chon.size > 0 && lyDo.trim().length >= 3 && (cach === "vong" || !!nhanId);
    ...
    <div className="mx-auto w-full max-w-[1180px] space-y-6 px-4 py-6 sm:px-6 min-[1536px]:max-w-[1440px] min-[2200px]:max-w-[1920px]">
      <header className="space-y-2">
        ...
        <p className="max-w-[78ch] rounded-xl border border-state-warning/40 bg-state-warning-soft px-4 py-3 text-xs text-state-warning-ink">
          Số ngày đếm từ lần gần nhất có người <b>ghi gì đó vào lead</b>: đổi trạng thái, thêm
          hoạt động, tạo hoặc đóng việc cần làm. Tư vấn viên nhắn Zalo/Messenger với khách mà
          không ghi lại thì hệ thống không thấy — hãy xem qua vài dòng trước khi phân bổ hàng
          loạt.
        </p>
      </header>
      ...
      <section className="flex flex-wrap items-end gap-3 rounded-xl border border-border bg-muted/40 p-4">
        ...
        <Button onClick={locLai} disabled={dangChay} variant="outline" className="min-h-11">
          <RefreshCw className={cn("mr-1.5 h-4 w-4", dangChay && "animate-spin")} />
          Lọc lại
        </Button>

        <p className="basis-full text-sm text-muted-foreground sm:basis-auto sm:ml-auto">
          Tìm thấy <b className="tabular-nums text-foreground">{tong}</b> lead
          {dong.length < tong && (
            <span className="text-xs"> · đang hiện {dong.length} dòng đầu</span>
          )}
        </p>
      </section>
                ...
                <thead>
                  <tr className="border-b border-border bg-muted">
                    <th scope="col" className="w-10 px-4 py-2.5">
                      <input
                        type="checkbox"
                        aria-label="Chọn hết các dòng đang hiện"
                        checked={dsTrang.length > 0 && dsTrang.every((d) => chon.has(d.id))}
                ...
                <tbody>
                  {dsTrang.map((d) => (
                    ...
                    >
                      ...
                      <td className="max-w-[26ch] truncate whitespace-nowrap px-4 py-3 font-medium" title={d.parentName}>
                        {d.parentName}
                      </td>
                      ...
                      <td className="max-w-[22ch] truncate whitespace-nowrap px-4 py-3" title={d.chuTen ?? ""}>
                        {d.chuTen ?? (
          ...
          {soTrang > 1 && (
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-xs text-muted-foreground">
                Dòng {(trangHt - 1) * SO_DONG_MOI_TRANG + 1}–
                {Math.min(trangHt * SO_DONG_MOI_TRANG, dong.length)} trên {dong.length}
              </p>
      ...
      {/* Thanh hành động dính đáy — danh sách dài thì nút quan trọng nhất vẫn trong tầm mắt.
      ...
      <div className="sticky bottom-0 z-20 -mx-4 space-y-3 border-t border-border bg-background/95 px-4 py-3 backdrop-blur supports-[backdrop-filter]:bg-background/80 sm:mx-0 sm:rounded-xl sm:border sm:px-4">
        ...
        <div className="flex flex-wrap gap-2">
          {/* Hai cách chia — chủ dự án chốt có cả hai. Bày ra cùng lúc để quản lý thấy được
              lựa chọn thứ hai tồn tại, thay vì giấu sau một menu. */}
          {(
            [
              { k: "vong" as const, nhan: "Chia vòng cho cả pool", Icon: Shuffle },
        ...
        <div className="flex flex-wrap items-center gap-2">
          {/* Lý do BẮT BUỘC: đây là thao tác giật lead khỏi tay người khác. Ba tháng sau người
              bị giật hỏi "vì sao" thì phải có câu trả lời. */}
          <input
            value={lyDo}
