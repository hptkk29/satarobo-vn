...
import { ADS_BUDGET_TARGET_AMOUNT_MAX } from "@/lib/reports/ads-budget-target";
import { setAdsBudgetTargetAction } from "../_actions";

type CenterOption = { id: string; name: string };

export function AdsBudgetTargetForm({
  centers,
  canSetGlobal,
  defaultCenterId,
  defaultPeriod,
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res = await setAdsBudgetTargetAction(formData);
      if (res.ok) {
        toast.success("Đã lưu chỉ tiêu ngân sách quảng cáo");
      } else {
  ...
  return (
    <form onSubmit={onSubmit} className="grid grid-cols-1 gap-3 md:grid-cols-5 md:items-end">
      ...
      <div className="space-y-1">
        ...
        <Input id="abt-period" name="period" type="month" defaultValue={defaultPeriod} required />
      </div>

      <div className="space-y-1">
        <Label htmlFor="abt-amount">
          Chỉ tiêu ngân sách (đ)
          <HelpHint>
            Số tiền ĐẶT RA để chi cho quảng cáo trong tháng, đơn vị đồng. Đây không phải
            số đã tiêu — số đã tiêu lấy tự động từ tài khoản quảng cáo. Nhập chữ số
            liền, không dấu chấm/phẩy: 30 triệu gõ là 30000000.
          </HelpHint>
        </Label>
        <Input
          id="abt-amount"
          ...
          type="number"
          min={0}
          max={ADS_BUDGET_TARGET_AMOUNT_MAX}
          step={1}
          inputMode="numeric"
          placeholder="0"
          required
      ...
      <div className="space-y-1">
        <Label htmlFor="abt-note">
          Ghi chú
          <HelpHint>
            Ghi lý do đặt mức này (VD: &ldquo;đẩy tuyển sinh hè, tăng 50% so với tháng
            5&rdquo;) để sau này nhìn lại còn hiểu vì sao.
          </HelpHint>
        </Label>
        <Input id="abt-note" name="note" type="text" placeholder="Tùy chọn" />
      </div>
