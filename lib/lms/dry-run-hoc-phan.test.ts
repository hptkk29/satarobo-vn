// Ca [DRH-*] — DRY-RUN HỌC PHẦN chạy trên PROD phải CHỈ ĐỌC. Thuần, không DB.
//
// ⚠️ Báo cáo này nguy hiểm hơn `bao-cao-hoc-phan.ts` một bậc: nó dựng lại phép `upsert`
// của seed. Dựng LẠI thì dễ trượt tay thành GỌI THẬT — nên bốn lớp khoá ở đây không phải
// thủ tục chép lại, chúng canh đúng cái rủi ro đó.
//
...
const SCRIPT = "scripts/dry-run-hoc-phan.ts";
const WORKFLOW = ".github/workflows/hoc-phan-dry-run-prod-chi-doc.yml";
...
describe("[DRH-01] script KHÔNG chứa một lệnh ghi nào", () => {
...
describe("[DRH-02] mọi truy vấn nằm TRONG transaction READ ONLY, và nó LUÔN rollback", () => {
  const ma = boChuThich(doc(SCRIPT));
...
describe("[DRH-03] workflow — bốn lớp khoá còn nguyên", () => {
...
describe("[DRH-04] báo cáo KHÔNG chở dữ liệu cá nhân ra ngoài", () => {
  const ma = boChuThich(doc(SCRIPT));
