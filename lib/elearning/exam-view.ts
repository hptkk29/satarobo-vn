 ...
 *
 * ⚠️ Trộn thứ tự làm Ở ĐÂY, mỗi lượt tải một khác — nhưng MÃ lựa chọn giữ nguyên
...
/** Trộn tại chỗ bằng Fisher–Yates. */
function tron<T>(a: T[]): T[] {
  const r = [...a];
  ...
  if (dangMo) {
    ...
    const dung: CauDeThiChoNguoiHoc[] = cacCau.map((c) => {
      ...
      return {
        ...
        luaChon: de.shuffleChoices && loc ? tron(loc.luaChon) : (loc?.luaChon ?? []),
    ...
    luotDangLam = {
      ...
      cacCau: de.shuffleQuestions ? tron(dung) : dung,
    };
  }
