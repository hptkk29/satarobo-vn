...
> 🔗 **Phụ lục chi tiết** cho bản chốt [`04-LMS-master-fix-plan.md`](./04-LMS-master-fix-plan.md). Khi ưu tiên/wave xung đột → bản 04 thắng. (Audit trên `full-R7` **pre-merge** — chưa có lớp erd-fix.)
>
...
> ✏️ **CẬP NHẬT (sau erd-fix):** 2 file `lib/enrollments/status.ts` + `lib/sessions/status.ts` **ĐÃ được erd-fix-p0 tạo** (đến cùng merge `main→full-R7`). Việc còn lại KHÔNG phải "tạo" mà là **nới guard `SCHEDULED→COMPLETED` cho buổi offline + nối 2 call-site chưa guard** (`assign.ts:140`, `students/_actions.ts:555`) — xem bản 04 **W2-1**.

Trùng ERD **FIX-H4**. ~~Tạo~~ `lib/enrollments/status.ts` + `lib/sessions/status.ts` (đã có); chặn nhảy phi lý (CANCELLED→ACTIVE, mở lại buổi COMPLETED).
