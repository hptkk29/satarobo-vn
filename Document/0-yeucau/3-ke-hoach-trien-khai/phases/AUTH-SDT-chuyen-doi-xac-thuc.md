...
> **✅ P3 HOÀN THÀNH 31/07** (nhánh `feat/auth-sdt-p3`): 2 migration additive đã áp (enum `ZALO` tách riêng theo tiền lệ; `User.phone @unique` + `phoneVerifiedAt` + email nullable giữ unique, KHÔNG backfill) · `loginSchema.identifier` + authorize **rẽ nhánh** phone/email theo `canonicalPhone` (không dùng OR — né hẳn bẫy `{phone: null}`) · rate-limit key `login:id:` chuẩn hoá trước · `phone` vào đủ 4 chỗ · form "Số điện thoại hoặc Email" (dùng `inputMode="email"` thay vì "tel" — bàn phím tel không có chữ cái, mobile sẽ không gõ nổi email) · **ngữ nghĩa hoa/thường email GIỮ NGUYÊN** — muốn đổi phải chạy `scripts/sql/login-email-case.sql` trên PROD trước (0 dòng = đổi được) · sweep xong `?? "Unknown"` → fallback `userId`, KHÔNG đưa SĐT vào actorName · `seedUser` tự sinh phone deterministic từ email (90 call-site giữ nguyên) · DoD: a0 90/90 (thêm 5 case [P3-C1..C5]) · smoke 22/22 (spec cập nhật theo ô identifier) · baseline 32 spec không hồi quy · unit +5 (identifier + khoá QĐ-C staff bắt buộc email). Ghi chú flake KHÔNG do P3: `[A0-02-T1-10]` (bộ đo nguyên tử seedRoles) nhạy tải CPU — polls=1 khi cả suite chạy trên máy nhanh, pass 3/3 khi chạy riêng.
...
>
> **CÒN NỢ (chặn DoD, chưa làm):** ① **backfill `User.phone`** — chưa viết chưa chạy; phụ huynh cũ có `User.phone = NULL` (P3 thêm cột nullable, **cố ý không backfill**) mà `lib/auth.ts:123` tra **đúng-bằng** `{ phone: canonical }` chứ không qua `phoneVariants` ⇒ **không đăng nhập bằng SĐT được**.
>   ⚠️ **Đừng nhầm với `scripts/phone-backfill.ts` đã có sẵn** — script đó là **canonical hoá P1**, chỉ đụng `Student.phone/parentPhone/parent2Phone`, **không hề ghi `User.phone`**. Chạy nó xong vẫn còn nguyên việc này.
>   💡 Nếu dọn sạch data vận hành trên prod (kế hoạch 01/08) thì **việc này tự biến mất** — không còn phụ huynh cũ để backfill, mọi tài khoản sinh sau đều đi đường P5 nên có `User.phone` ngay từ đầu ② chạy `scripts/phone-audit.ts` trên **PROD** lấy số nhóm SĐT trùng (điều kiện *vào* phase, làm ngược thứ tự) ③ đóng hết `ConvertConflict status=OPEN` ④ **DoD "phụ huynh nhận đúng 1 tin ZNS trên máy" không nghiệm thu được trên `test`** (creds Zalo chỉ ở scope Production, cấm nhân bản `ZALO_OA_REFRESH_TOKEN`) → phải smoke trên prod ngay sau merge, đừng để trôi vì "test đã xanh".
>
