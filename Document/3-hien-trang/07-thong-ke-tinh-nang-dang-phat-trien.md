...
3. 🔴→✅ **Bucket PROD `satarobo-assets` từng THIẾU origin `giaovien.satarobo.vn` — ĐÃ VÁ 18/08.** Đo bằng preflight OPTIONS+PUT thật (không cần creds): trước vá, các origin khác 204 + ACAO nhưng **giaovien → 403** (CORS áp từ TRƯỚC khi site GV ra đời 07/2026, chưa refresh theo `scripts/r2-cors.json`) ⇒ GV thuần trên PROD không upload được ảnh lớp, "chết câm" y hệt sự cố bucket test/chat. **Đo lại sau vá: giaovien/admin/hocvien/satarobo.vn/www/localhost đều 204 + ACAO + đủ method PUT.** Ghi chú: origin `test.satarobo.vn` vẫn 403 trên bucket PROD — vô hại (env test dùng bucket riêng `satarobo-test`). `satarobo-chat` + `satarobo-test` đủ origin từ trước (đo cùng lúc).

**Việc còn treo của module:**

- ✅ ~~Áp lại CORS cho bucket PROD `satarobo-assets`~~ — **vá xong 18/08** (dán qua Cloudflare dashboard; token `.env.local`/`.env.r2test` không có quyền `PutBucketCors`), đo lại preflight PASS đủ origin prod. Tuỳ chọn: smoke 1 lần upload ảnh thật từ site GV prod khi tiện.
